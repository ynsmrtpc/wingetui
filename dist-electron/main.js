...
import { screen, app, ipcMain, BrowserWindow } from "electron";
...
import require$$1$1, { spawn as spawn$1 } from "child_process";
...
class SystemInfoService {
  ...
  async getNetworkUsage() {
    try {
      ...
      if (networkStats2 && networkStats2.length > 0) {
        const mainNetwork = networkStats2.find((net2) => net2.operstate === "up") || networkStats2[0];
        return {
          interface: mainNetwork.iface,
          download: Math.round(mainNetwork.rx_sec / 1024),
          // KB/s
          upload: Math.round(mainNetwork.tx_sec / 1024),
          // KB/s
          usage: Math.round((mainNetwork.rx_sec + mainNetwork.tx_sec) / (10 * 1024 * 1024) * 100)
          // % of 10 Mbps
        };
      }
      return {
   ...
   */
  calculatePerformanceScore(cpuUsage, memoryUsage, gpuUsage, networkUsage) {
...
class WingetService {
  ...
  async installWinget() {
    try {
      ...
      const process2 = spawn$1("cmd.exe", ["/c", "start", "ms-appinstaller:?source=winget"]);
      return new Promise((resolve) => {
        process2.on("error", (error) => {
  ...
  async uninstallApp(appId) {
    try {
      ...
      const installedApps = await this.getInstalledApps();
      const isInstalled = installedApps.some((app2) => app2.id === appId);
      if (!isInstalled) {
        return {
          success: false,
      ...
      const result2 = await this.executeCommand("uninstall", [appId, "--accept-source-agreements"]);
      if (result2.includes("successfully") || result2.includes("başarıyla")) {
        console.log(`Successfully uninstalled app: ${appId}`);
        const updatedInstalledApps = await this.getInstalledApps();
        const stillInstalled = updatedInstalledApps.some((app2) => app2.id === appId);
        if (stillInstalled) {
        ...
        return {
          success: true,
          message: `${appId} kaldırıldı: ${result2.split("\n")[0]}`
        };
      } else {
        console.error(`Error uninstalling app: ${appId}`, result2);
        return {
          success: false,
          message: `${appId} kaldırılamadı: ${result2.split("\n")[0]}`
        };
      }
    } catch (error) {
...
class IpcHandlerService {
  ...
  registerWingetHandlers() {
    ipcMain.handle("get-installed-apps", async () => {
      try {
        const result2 = await this.wingetService.getInstalledApps();
        ...
        console.error("Unexpected result format from getInstalledApps:", result2);
    ...
    ipcMain.handle("search-apps", async (_event, keyword) => {
      try {
        const result2 = await this.wingetService.searchApps(keyword);
        if (result2 && typeof result2 === "object" && "error" in result2) {
          return result2;
        }
        if (Array.isArray(result2)) {
        ...
        console.error("Unexpected result format from searchApps:", result2);
    ...
    ipcMain.handle("update-app", async (_event, appId) => {
      try {
        ...
        if (result2 && typeof result2 === "object" && "success" in result2) {
          return result2;
        } else {
    ...
    ipcMain.handle("uninstall-app", async (_event, appId) => {
      try {
        const result2 = await this.wingetService.uninstallApp(appId);
        if (result2 && typeof result2 === "object" && "success" in result2) {
          return result2;
        } else {
          return { success: true, message: result2 };
    ...
    ipcMain.handle("install-winget", async () => {
      try {
        const result2 = await this.wingetService.installWinget();
        return result2;
      } catch (error) {
...
const iconPath = path$3.join(process.env.APP_ROOT, "public", "icon.svg");
let win;
