...
var fo = (t, i, e) => i in t ? po(t, i, { enumerable: !0, configurable: !0, writable: !0, value: e }) : t[i] = e;
var Ge = (t, i, e) => fo(t, typeof i != "symbol" ? i + "" : i, e);
import { screen as mo, app as wi, ipcMain as oi, BrowserWindow as ms } from "electron";
import { createRequire as ho } from "node:module";
import { fileURLToPath as go } from "node:url";
import tt from "node:path";
import oe, { platform as xo } from "os";
import Fe from "fs";
import Xn from "path";
import te, { spawn as wr } from "child_process";
import yo from "util";
import So from "https";
import wo from "http";
import Co from "net";
import Lo from "buffer";
import vo from "string_decoder";
...
var T = {};
const Qe = oe, Ne = Fe, Oo = Xn, Kn = te.spawn, Po = te.exec, Jt = te.execSync, Ao = yo;
let Dt = process.platform;
const Yn = Dt === "linux" || Dt === "android", gs = Dt === "darwin", Ji = Dt === "win32", xs = Dt === "freebsd", ys = Dt === "openbsd", Ss = Dt === "netbsd";
let xn = 0, Pt = "", De = "", Ke = null, Ve = null;
const Jn = process.env.WINDIR || "C:\\Windows";
let pe, Ft = "", di = [], Qn = !1, $i = "";
const Cr = "$OutputEncoding = [System.Console]::OutputEncoding = [System.Console]::InputEncoding = [System.Text.Encoding]::UTF8 ; ", Rn = "--###START###--", Lr = "--ERROR--", Hi = "--###ENDCMD###--", Gn = "--##ID##--", Oi = {
  windowsHide: !0,
  maxBuffer: 1024 * 2e4,
  encoding: "UTF-8",
...
const Ri = new String(), Un = new String().replace, zn = new String().toLowerCase, ws = new String().toString, Cs = new String().substr, Ls = new String().substring, vs = new String().trim, Is = new String().startsWith, bs = Math.min;
function To(t) {
...
function Vo() {
  return xn === 0 && (xn = Qe.cpus().length), xn;
}
...
function Fo(t, i) {
  i = i || "", t = t.toUpperCase();
  let e = 0, n = 0, s = ko(t), r = t.split(s);
  if (r.length >= 2) {
    r[2] && (r[1] += r[2]);
...
function Uo() {
  if ($i = "powershell.exe", Ji) {
    const t = `${Jn}\\system32\\WindowsPowerShell\\v1.0\\powershell.exe`;
    Ne.existsSync(t) && ($i = t);
  }
}
function _s() {
  if (Qe.type() === "Windows_NT" && !Pt && (Pt = Jn + "\\system32\\wbem\\wmic.exe", !Ne.existsSync(Pt)))
    try {
      const t = Jt("WHERE WMIC", Oi).toString().split(`\r
`);
...
function $o() {
  return Ji ? `"${process.env.VBOX_INSTALL_PATH || process.env.VBOX_MSI_INSTALL_PATH}\\VBoxManage.exe"` : "vboxmanage";
}
function yn(t) {
  let i = "", e, n = "";
  if (t.indexOf(Rn) >= 0) {
    e = t.split(Rn);
    const r = e[1].split(Gn);
    i = r[0], r.length > 1 && (t = r.slice(1).join(Gn));
  }
  t.indexOf(Hi) >= 0 && (e = t.split(Hi), n = e[0]);
  let s = -1;
...
function Ho() {
  pe || (pe = Kn($i, ["-NoProfile", "-NoLogo", "-InputFormat", "Text", "-NoExit", "-Command", "-"], {
    stdio: "pipe",
    windowsHide: !0,
    maxBuffer: 1024 * 2e4,
    encoding: "UTF-8",
    env: Object.assign({}, process.env, { LANG: "en_US.UTF-8" })
  }), pe && pe.pid && (Qn = !0, pe.stdout.on("data", function(t) {
    Ft = Ft + t.toString("utf8"), t.indexOf(Hi) >= 0 && (yn(Ft), Ft = "");
...
function qo() {
  try {
    pe && (pe.stdin.write("exit" + Qe.EOL), pe.stdin.end(), Qn = !1);
  } catch {
    pe && pe.kill();
  }
  pe = null;
  ...
  if (Qn) {
    ...
    return new Promise((e) => {
      process.nextTick(() => {
        ...
        try {
          pe && pe.pid && pe.stdin.write(Cr + "echo " + Rn + i + Gn + "; " + Qe.EOL + t + Qe.EOL + "echo " + Hi + Qe.EOL);
        } catch {
          e("");
        }
      });
    ...
    return new Promise((e) => {
      process.nextTick(() => {
        try {
          const n = Kn($i, ["-NoProfile", "-NoLogo", "-InputFormat", "Text", "-NoExit", "-ExecutionPolicy", "Unrestricted", "-Command", "-"], {
            stdio: "pipe",
            windowsHide: !0,
            maxBuffer: 2048e4,
            encoding: "UTF-8",
          ...
          if (n && !n.pid && n.on("error", function() {
            ...
            try {
              n.stdin.write(Cr + t + Qe.EOL), n.stdin.write("exit" + Qe.EOL), n.stdin.end();
            } catch {
              n.kill(), e(i);
            }
          } else
...
function Xo() {
  if (Ji) {
    if (!De)
      try {
        const e = Jt("chcp", Oi).toString().split(`\r
`)[0].split(":");
  ...
  if (Yn || gs || xs || ys || Ss) {
    if (!De)
      try {
        const e = Jt("echo $LANG", Qi).toString().split(`\r
`)[0].split(".");
...
function Ko() {
  ...
  if (Ke = !1, Ji)
    try {
      const t = Jt("WHERE smartctl 2>nul", Oi).toString().split(`\r
`);
  ...
  if (Yn || gs || xs || ys || Ss)
    try {
      Ke = Jt("which smartctl 2>/dev/null", Qi).toString().split(`\r
`).length > 0;
    } catch {
      Ao.noop();
    }
...
function Yo(t) {
  ...
  if (Ve !== null)
    ...
    try {
      t = Ne.readFileSync("/proc/cpuinfo", { encoding: "utf8" }).toString().split(`
`), Ve = t;
    } catch {
      return !1;
    }
...
function Jo() {
  let t = [];
  try {
    t = Ne.readFileSync("/etc/os-release", { encoding: "utf8" }).toString().split(`
`);
  } catch {
    return !1;
  }
...
function Zo() {
  const t = Ne.existsSync("/Library/Developer/CommandLineTools/usr/bin/"), i = Ne.existsSync("/Applications/Xcode.app/Contents/Developer/Tools"), e = Ne.existsSync("/Library/Developer/Xcode/");
  return t || e || i;
}
function ea() {
...
function ra() {
  ...
  let i = !0, e = "";
  try {
    e.__proto__.replace = Un, e.__proto__.toLowerCase = zn, e.__proto__.toString = ws, e.__proto__.substr = Cs, e.__proto__.substring = Ls, e.__proto__.trim = vs, e.__proto__.startsWith = Is;
  } catch {
    Object.setPrototypeOf(e, Ri);
  }
  i = i || t.length !== 62;
  const n = Date.now();
  if (typeof n == "number" && n > 16e11) {
    ...
    try {
      o.__proto__.replace = Un;
    } catch {
      Object.setPrototypeOf(o, Ri);
    }
    let a = o.replace(/ /g, "");
...
function oa(t) {
  const i = Ne.lstatSync, e = Ne.readdirSync, n = Oo.join;
  function s(c) {
...
function Ps(t) {
  ...
  if ({}.hasOwnProperty.call(i, r))
    ...
    const c = ("00000000" + Ze(t, "revision", ":", !0).toLowerCase()).substr(-8), u = parseInt(sa(c.substr(2, 1)).substr(5, 3), 2) || 0, f = n[parseInt(c.substr(3, 1), 10)], p = e[parseInt(c.substr(4, 1), 10)], d = c.substr(5, 2);
    l = {
      model: o,
      serial: a,
      revisionCode: r,
...
function ga(t, i = 5e3) {
  const e = t.startsWith("https:") || t.indexOf(":443/") > 0 || t.indexOf(":8443/") > 0 ? So : wo, n = Date.now();
...
T.parseDateTime = Wo;
T.parseHead = Ro;
T.findObjectByKey = Go;
T.getWmic = _s;
T.wmic = zo;
T.darwinXcodeExists = Zo;
T.getVboxmanage = $o;
T.powerShell = Os;
T.powerShellStart = Ho;
T.powerShellRelease = qo;
T.execSafe = jo;
T.nanoSeconds = ea;
T.countUniqueLines = ta;
T.countLines = ia;
T.noop = ya;
T.isRaspberry = Yo;
T.isRaspbian = Jo;
T.sanitizeShellString = na;
T.isPrototypePolluted = ra;
T.decodePiCpuinfo = Ps;
T.getRpiGpu = aa;
T.promiseAll = ca;
T.promisify = la;
T.promisifySave = ua;
T.smartMonToolsInstalled = Ko;
T.linuxVersion = pa;
T.plistParser = fa;
T.plistReader = da;
T.stringObj = Ri;
...
T.getPowershell = Uo;
var Pi = {};
const Sn = Fe, Et = oe, L = T, Qt = te.exec, qt = te.execSync, Mi = L.promisify(te.exec);
let nt = process.platform;
const Zi = nt === "linux" || nt === "android", en = nt === "darwin", tn = nt === "win32", Ci = nt === "freebsd", Li = nt === "openbsd", vi = nt === "netbsd", nn = nt === "sunos";
function Sa(t) {
  return new Promise((i) => {
    process.nextTick(() => {
      let e = {
      ...
      if ((Zi || Ci || Li || vi) && Qt("export LC_ALL=C; dmidecode -t system 2>/dev/null; unset LC_ALL", function(n, s) {
        let r = s.toString().split(`
`);
        e.manufacturer = j(L.getValue(r, "manufacturer")), e.model = j(L.getValue(r, "product name")), e.version = j(L.getValue(r, "version")), e.serial = j(L.getValue(r, "serial number")), e.uuid = j(L.getValue(r, "uuid")).toLowerCase(), e.sku = j(L.getValue(r, "sku number"));
        ...
        try {
          r = qt(o, L.execOptsLinux).toString().split(`
`), e.manufacturer = j(e.manufacturer === "" ? L.getValue(r, "sys_vendor") : e.manufacturer), e.model = j(e.model === "" ? L.getValue(r, "product_name") : e.model), e.version = j(e.version === "" ? L.getValue(r, "product_version") : e.version), e.serial = j(e.serial === "" ? L.getValue(r, "product_serial") : e.serial), e.uuid = j(e.uuid === "" ? L.getValue(r, "product_uuid").toLowerCase() : e.uuid);
        } catch {
          L.noop();
        }
        ...
        if (!e.virtual)
          try {
            const a = qt("ls -1 /dev/disk/by-id/ 2>/dev/null", L.execOptsLinux).toString();
        ...
        if ((Ci || Li || vi) && !e.virtualHost)
          try {
            const l = qt("dmidecode -t 4", L.execOptsLinux).toString().split(`
`);
            switch (L.getValue(l, "manufacturer", ":", !0).toLowerCase()) {
              case "virtualbox":
                e.virtualHost = "VirtualBox";
                break;
              case "vmware":
        ...
        (Sn.existsSync("/.dockerenv") || Sn.existsSync("/.dockerinit")) && (e.model = "Docker Container");
        try {
          const a = qt('dmesg 2>/dev/null | grep -iE "virtual|hypervisor" | grep -iE "vmware|qemu|kvm|xen" | grep -viE "Nested Virtualization|/virtual/"');
          a.toString().split(`
`).length > 0 && (e.model === "Computer" && (e.model = "Virtual machine"), e.virtual = !0, a.toString().toLowerCase().indexOf("vmware") >= 0 && !e.virtualHost && (e.virtualHost = "VMware"), a.toString().toLowerCase().indexOf("qemu") >= 0 && !e.virtualHost && (e.virtualHost = "QEMU"), a.toString().toLowerCase().indexOf("xen") >= 0 && !e.virtualHost && (e.virtualHost = "Xen"), a.toString().toLowerCase().indexOf("kvm") >= 0 && !e.virtualHost && (e.virtualHost = "KVM"));
        ...
        e.manufacturer === "" && e.model === "Computer" && e.version === "" ? Sn.readFile("/proc/cpuinfo", function(a, l) {
          if (!a) {
            let c = l.toString().split(`
`);
            if (e.model = L.getValue(c, "hardware", ":", !0).toUpperCase(), e.version = L.getValue(c, "revision", ":", !0).toLowerCase(), e.serial = L.getValue(c, "serial", ":", !0), L.getValue(c, "model:", ":", !0), L.isRaspberry(c)) {
              const u = L.decodePiCpuinfo(c);
      ...
      }), en && Qt("ioreg -c IOPlatformExpertDevice -d 2", function(n, s) {
        if (!n) {
          let r = s.toString().replace(/[<>"]/g, "").split(`
`);
          const o = L.getAppleModel(L.getValue(r, "model", "=", !0));
          e.manufacturer = L.getValue(r, "manufacturer", "=", !0), e.model = o.key, e.type = As(o.version), e.version = o.version, e.serial = L.getValue(r, "ioplatformserialnumber", "=", !0), e.uuid = L.getValue(r, "ioplatformuuid", "=", !0).toLowerCase(), e.sku = L.getValue(r, "board-id", "=", !0) || L.getValue(r, "target-sub-type", "=", !0);
        }
        t && t(e), i(e);
      }), nn && (t && t(e), i(e)), tn)
        try {
          L.powerShell("Get-CimInstance Win32_ComputerSystemProduct | select Name,Vendor,Version,IdentifyingNumber,UUID | fl").then((n, s) => {
            if (s)
              t && t(e), i(e);
            else {
              let r = n.split(`\r
`);
              e.manufacturer = L.getValue(r, "vendor", ":"), e.model = L.getValue(r, "name", ":"), e.version = L.getValue(r, "version", ":"), e.serial = L.getValue(r, "identifyingnumber", ":"), e.uuid = L.getValue(r, "uuid", ":").toLowerCase();
...
`);
                ...
                e.virtual ? (t && t(e), i(e)) : L.powerShell("Get-CimInstance Win32_bios | select Version, SerialNumber, SMBIOSBIOSVersion").then((u, f) => {
                  if (f)
                    t && t(e), i(e);
                  else {
                    let p = u.toString();
...
function wa(t) {
  return new Promise((i) => {
    process.nextTick(() => {
      ...
      if ((Zi || Ci || Li || vi) && (process.arch === "arm" ? n = "cat /proc/cpuinfo | grep Serial" : n = "export LC_ALL=C; dmidecode -t bios 2>/dev/null; unset LC_ALL", Qt(n, function(s, r) {
...
`);
        ...
        let l = L.getValue(o, "Currently Installed Language").split("|")[0];
        ...
        try {
          o = qt(c, L.execOptsLinux).toString().split(`
`), e.vendor = e.vendor ? e.vendor : L.getValue(o, "bios_vendor"), e.version = e.version ? e.version : L.getValue(o, "bios_version"), a = L.getValue(o, "bios_date"), e.releaseDate = e.releaseDate ? e.releaseDate : L.parseDateTime(a).date;
      ...
      )), nn && (e.vendor = "Sun Microsystems", t && t(e), i(e)), tn)
        try {
          L.powerShell('Get-CimInstance Win32_bios | select Description,Version,Manufacturer,@{n="ReleaseDate";e={$_.ReleaseDate.ToString("yyyy-MM-dd")}},BuildNumber,SerialNumber,SMBIOSBIOSVersion | fl').then((s, r) => {
            if (!r) {
              let o = s.toString().split(`\r
`);
              const a = L.getValue(o, "description", ":"), l = L.getValue(o, "SMBIOSBIOSVersion", ":");
              a.indexOf(" Version ") !== -1 ? (e.vendor = a.split(" Version ")[0].trim(), e.version = a.split(" Version ")[1].trim()) : a.indexOf(" Ver: ") !== -1 ? (e.vendor = L.getValue(o, "manufacturer", ":"), e.version = a.split(" Ver: ")[1].trim()) : (e.vendor = L.getValue(o, "manufacturer", ":"), e.version = l || L.getValue(o, "version", ":")), e.releaseDate = L.getValue(o, "releasedate", ":"), e.revision = L.getValue(o, "buildnumber", ":"), e.serial = j(L.getValue(o, "serialnumber", ":"));
            }
            t && t(e), i(e);
          });
        } catch {
  ...
  return new Promise((i) => {
    process.nextTick(() => {
      ...
      if (Zi || Ci || Li || vi) {
        ...
        const s = [];
        s.push(Mi(n)), s.push(Mi("export LC_ALL=C; dmidecode -t memory 2>/dev/null")), L.promiseAll(
          s
        ).then((r) => {
          let o = r.results[0] ? r.results[0].toString().split(`
`) : [""];
          e.manufacturer = j(L.getValue(o, "Manufacturer")), e.model = j(L.getValue(o, "Product Name")), e.version = j(L.getValue(o, "Version")), e.serial = j(L.getValue(o, "Serial Number")), e.assetTag = j(L.getValue(o, "Asset Tag"));
          ...
          try {
            o = qt(a, L.execOptsLinux).toString().split(`
`), e.manufacturer = j(e.manufacturer ? e.manufacturer : L.getValue(o, "board_vendor")), e.model = j(e.model ? e.model : L.getValue(o, "board_name")), e.version = j(e.version ? e.version : L.getValue(o, "board_version")), e.serial = j(e.serial ? e.serial : L.getValue(o, "board_serial")), e.assetTag = j(e.assetTag ? e.assetTag : L.getValue(o, "board_asset_tag"));
          } catch {
            L.noop();
          }
...
`) : [""], e.memMax = L.toInt(L.getValue(o, "Maximum Capacity")) * 1024 * 1024 * 1024 || null, e.memSlots = L.toInt(L.getValue(o, "Number Of Devices")) || null, L.isRaspberry()) {
      ...
      if (en) {
        const s = [];
        s.push(Mi("ioreg -c IOPlatformExpertDevice -d 2")), s.push(Mi("system_profiler SPMemoryDataType")), L.promiseAll(
          s
        ).then((r) => {
...
`) : [""];
      ...
      if (nn && (t && t(e), i(e)), tn)
        try {
          ...
          s.push(L.powerShell("Get-CimInstance Win32_baseboard | select Model,Manufacturer,Product,Version,SerialNumber,PartNumber,SKU | fl")), s.push(L.powerShell(`Get-CimInstance Win32_physicalmemoryarray | select ${o}, MemoryDevices | fl`)), L.promiseAll(
            s
          ).then((a) => {
            let l = a.results[0] ? a.results[0].toString().split(`\r
`) : [""];
            e.manufacturer = j(L.getValue(l, "manufacturer", ":")), e.model = j(L.getValue(l, "model", ":")), e.model || (e.model = j(L.getValue(l, "product", ":"))), e.version = j(L.getValue(l, "version", ":")), e.serial = j(L.getValue(l, "serialnumber", ":")), e.assetTag = j(L.getValue(l, "partnumber", ":")), e.assetTag || (e.assetTag = j(L.getValue(l, "sku", ":"))), l = a.results[1] ? a.results[1].toString().split(`\r
`) : [""], e.memMax = L.toInt(L.getValue(l, o, ":")) * (r ? 1024 : 1) || null, e.memSlots = L.toInt(L.getValue(l, "MemoryDevices", ":")) || null, t && t(e), i(e);
          });
        } catch {
          t && t(e), i(e);
        }
...
function La(t) {
  ...
  return new Promise((e) => {
    process.nextTick(() => {
      ...
      if ((Zi || Ci || Li || vi) && Qt(`echo -n "chassis_asset_tag: "; cat /sys/devices/virtual/dmi/id/chassis_asset_tag 2>/dev/null; echo;
...
`);
        n.manufacturer = j(L.getValue(a, "chassis_vendor"));
        const l = parseInt(L.getValue(a, "chassis_type").replace(/\D/g, ""));
        n.type = j(l && !isNaN(l) && l < i.length ? i[l - 1] : ""), n.version = j(L.getValue(a, "chassis_version")), n.serial = j(L.getValue(a, "chassis_serial")), n.assetTag = j(L.getValue(a, "chassis_asset_tag")), t && t(n), e(n);
      }), en && Qt("ioreg -c IOPlatformExpertDevice -d 2", function(s, r) {
        if (!s) {
          let o = r.toString().replace(/[<>"]/g, "").split(`
`);
          const a = L.getAppleModel(L.getValue(o, "model", "=", !0));
          n.manufacturer = L.getValue(o, "manufacturer", "=", !0), n.model = a.key, n.type = As(a.model), n.version = a.version, n.serial = L.getValue(o, "ioplatformserialnumber", "=", !0), n.assetTag = L.getValue(o, "board-id", "=", !0) || L.getValue(o, "target-type", "=", !0), n.sku = L.getValue(o, "target-sub-type", "=", !0);
        }
        t && t(n), e(n);
      }), nn && (t && t(n), e(n)), tn)
        try {
          L.powerShell("Get-CimInstance Win32_SystemEnclosure | select Model,Manufacturer,ChassisTypes,Version,SerialNumber,PartNumber,SKU,SMBIOSAssetTag | fl").then((s, r) => {
            if (!r) {
              let o = s.toString().split(`\r
`);
              n.manufacturer = j(L.getValue(o, "manufacturer", ":")), n.model = j(L.getValue(o, "model", ":"));
              const a = parseInt(L.getValue(o, "ChassisTypes", ":").replace(/\D/g, ""));
              n.type = a && !isNaN(a) && a < i.length ? i[a - 1] : "", n.version = j(L.getValue(o, "version", ":")), n.serial = j(L.getValue(o, "serialnumber", ":")), n.assetTag = j(L.getValue(o, "partnumber", ":")), n.assetTag || (n.assetTag = j(L.getValue(o, "SMBIOSAssetTag", ":"))), n.sku = j(L.getValue(o, "sku", ":"));
            }
            t && t(n), e(n);
          });
        } catch {
...
Pi.chassis = La;
var ei = {};
const Me = oe, Ce = Fe, k = T, F = te.exec, hi = te.execSync;
let ke = process.platform;
const ti = ke === "linux" || ke === "android", Le = ke === "darwin", ve = ke === "win32", Zn = ke === "freebsd", er = ke === "openbsd", tr = ke === "netbsd", va = ke === "sunos";
function Ia() {
  ...
  if (Le || ti)
    try {
      const n = hi("date +%Z && date +%z && ls -l /etc/localtime 2>/dev/null", k.execOptsLinux).toString().split(Me.EOL);
...
function ba() {
  ...
  if (ve)
    try {
      t = hi("echo %COMPUTERNAME%.%USERDNSDOMAIN%", k.execOptsWin).toString().replace(".%USERDNSDOMAIN%", "").split(Me.EOL)[0];
    } catch {
      k.noop();
    }
  return t;
...
function _a(t) {
  return new Promise((i) => {
    process.nextTick(() => {
      ...
      if (ti && F("cat /etc/*-release; cat /usr/lib/os-release; cat /etc/openwrt_release", function(n, s) {
        let r = {};
        s.toString().split(`
`).forEach(function(u) {
        ...
        c.indexOf(e.distro + " ") === 0 && (a = c.replace(e.distro + " ", "").trim()), a.indexOf("(") >= 0 && (l = a.split("(")[1].replace(/[()]/g, "").trim(), a = a.split("(")[0].trim()), e.release = (a || r.DISTRIB_RELEASE || r.VERSION_ID || "unknown").replace(/"/g, ""), e.codename = l, e.codepage = k.getCodepage(), e.build = (r.BUILD_ID || "").replace(/"/g, "").trim(), Oa().then((u) => {
          e.uefi = u, Es().then((f) => {
            e.serial = f.os, t && t(e), i(e);
          });
        });
      }), (Zn || er || tr) && F("sysctl kern.ostype kern.osrelease kern.osrevision kern.hostuuid machdep.bootmethod kern.geom.confxml", function(n, s) {
        let r = s.toString().split(`
`);
        const o = k.getValue(r, "kern.ostype"), a = ai(o), l = k.getValue(r, "kern.osrelease").split("-")[0], c = k.getValue(r, "kern.uuid"), u = k.getValue(r, "machdep.bootmethod"), f = s.toString().indexOf("<type>efi</type>") >= 0, p = u ? u.toLowerCase().indexOf("uefi") >= 0 : f || null;
        e.distro = o || e.distro, e.logofile = a || e.logofile, e.release = l || e.release, e.serial = c || e.serial, e.codename = "", e.codepage = k.getCodepage(), e.uefi = p || null, t && t(e), i(e);
      }), Le && F("sw_vers; sysctl kern.ostype kern.osrelease kern.osrevision kern.uuid", function(n, s) {
        let r = s.toString().split(`
`);
        e.serial = k.getValue(r, "kern.uuid"), e.distro = k.getValue(r, "ProductName"), e.release = (k.getValue(r, "ProductVersion", ":", !0, !0) + " " + k.getValue(r, "ProductVersionExtra", ":", !0, !0)).trim(), e.build = k.getValue(r, "BuildVersion"), e.logofile = ai(e.distro), e.codename = "macOS", e.codename = e.release.indexOf("10.4") > -1 ? "OS X Tiger" : e.codename, e.codename = e.release.indexOf("10.5") > -1 ? "OS X Leopard" : e.codename, e.codename = e.release.indexOf("10.6") > -1 ? "OS X Snow Leopard" : e.codename, e.codename = e.release.indexOf("10.7") > -1 ? "OS X Lion" : e.codename, e.codename = e.release.indexOf("10.8") > -1 ? "OS X Mountain Lion" : e.codename, e.codename = e.release.indexOf("10.9") > -1 ? "OS X Mavericks" : e.codename, e.codename = e.release.indexOf("10.10") > -1 ? "OS X Yosemite" : e.codename, e.codename = e.release.indexOf("10.11") > -1 ? "OS X El Capitan" : e.codename, e.codename = e.release.indexOf("10.12") > -1 ? "Sierra" : e.codename, e.codename = e.release.indexOf("10.13") > -1 ? "High Sierra" : e.codename, e.codename = e.release.indexOf("10.14") > -1 ? "Mojave" : e.codename, e.codename = e.release.indexOf("10.15") > -1 ? "Catalina" : e.codename, e.codename = e.release.startsWith("11.") ? "Big Sur" : e.codename, e.codename = e.release.startsWith("12.") ? "Monterey" : e.codename, e.codename = e.release.startsWith("13.") ? "Ventura" : e.codename, e.codename = e.release.startsWith("14.") ? "Sonoma" : e.codename, e.codename = e.release.startsWith("15.") ? "Sequoia" : e.codename, e.uefi = !0, e.codepage = k.getCodepage(), t && t(e), i(e);
      }), va && (e.release = e.kernel, F("uname -o", function(n, s) {
        let r = s.toString().split(`
`);
        ...
        try {
          const n = [];
          n.push(k.powerShell("Get-CimInstance Win32_OperatingSystem | select Caption,SerialNumber,BuildNumber,ServicePackMajorVersion,ServicePackMinorVersion | fl")), n.push(k.powerShell("(Get-CimInstance Win32_ComputerSystem).HypervisorPresent")), n.push(k.powerShell("Add-Type -AssemblyName System.Windows.Forms; [System.Windows.Forms.SystemInformation]::TerminalServerSession")), k.promiseAll(
            n
          ).then((s) => {
            let r = s.results[0] ? s.results[0].toString().split(`\r
`) : [""];
            e.distro = k.getValue(r, "Caption", ":").trim(), e.serial = k.getValue(r, "SerialNumber", ":").trim(), e.build = k.getValue(r, "BuildNumber", ":").trim(), e.servicepack = k.getValue(r, "ServicePackMajorVersion", ":").trim() + "." + k.getValue(r, "ServicePackMinorVersion", ":").trim(), e.codepage = k.getCodepage();
            ...
            e.remoteSession = a.toString().toLowerCase().indexOf("true") >= 0, Pa().then((l) => {
              e.uefi = l, t && t(e), i(e);
            });
          });
        } catch {
...
function Pa() {
  return new Promise((t) => {
    process.nextTick(() => {
      try {
        F('findstr /C:"Detected boot environment" "%windir%\\Panther\\setupact.log"', k.execOptsWin, function(i, e) {
          if (i)
            F("echo %firmware_type%", k.execOptsWin, function(n, s) {
              if (n)
                return t(!1);
              {
                const r = s.toString() || "";
...
`)[0]), a();
        }), {}.hasOwnProperty.call(r.versions, "pm2") && (l = "pm2", ve && (l += ".cmd"), F(`${l} -v`, function(c, u) {
          if (!c) {
...
`)[0]), a();
        }), {}.hasOwnProperty.call(r.versions, "gulp") && (l = "gulp", ve && (l += ".cmd"), F(`${l} --version`, function(c, u) {
          if (!c) {
...
`)[0] || "";
          ...
          a();
        })), {}.hasOwnProperty.call(r.versions, "tsc") && (l = "tsc", ve && (l += ".cmd"), F(`${l} --version`, function(c, u) {
          if (!c) {
            const f = u.toString().split(`
`)[0] || "";
          ...
          a();
        })), {}.hasOwnProperty.call(r.versions, "grunt") && (l = "grunt", ve && (l += ".cmd"), F(`${l} --version`, function(c, u) {
          if (!c) {
            const f = u.toString().split(`
`)[0] || "";
          ...
          if (Le) {
            ...
            k.darwinXcodeExists() || c ? F("git --version", function(u, f) {
              if (!u) {
...
`)[0] || "").split(" ");
            r.versions.redis = k.getValue(p, "v", "=", !0);
          }
          a();
...
`)[0] || "").split(" ");
          ...
          if (!c) {
            const f = u.toString().split(`
`) || [];
            r.versions.postfix = k.getValue(f, "mail_version", "=", !0);
          }
          a();
...
`)[0] || "";
          ...
          a();
        }), {}.hasOwnProperty.call(r.versions, "postgresql") && (ti ? F("locate bin/postgres", function(c, u) {
          if (c)
...
`)[0].split(" ") || [];
        ...
        }) : ve ? k.powerShell("Get-CimInstance Win32_Service | select caption | fl").then((c) => {
          c.split(/\n\s*\n/).forEach((f) => {
            if (f.trim() !== "") {
              let p = f.trim().split(`\r
`), d = k.getValue(p, "caption", ":", !0).toLowerCase();
...
`), f = k.getValue(u, "ProductVersion", ":"), p = Ce.existsSync("/usr/local/Cellar/python"), d = Ce.existsSync("/opt/homebrew/bin/python");
              k.darwinXcodeExists() && k.semverCompare("12.0.1", f) < 0 || p || d ? F(p ? "/usr/local/Cellar/python -V 2>&1" : d ? "/opt/homebrew/bin/python -V 2>&1" : "python -V 2>&1", function(h, x) {
                if (!h) {
...
`)[0] || "";
        ...
        if ({}.hasOwnProperty.call(r.versions, "python3"))
          if (Le) {
            ...
            k.darwinXcodeExists() || c ? F("python3 -V 2>&1", function(u, f) {
              if (!u) {
...
`)[0] || "";
        ...
        if ({}.hasOwnProperty.call(r.versions, "pip"))
          if (Le) {
            ...
            k.darwinXcodeExists() || c ? F("pip -V 2>&1", function(u, f) {
              if (!u) {
...
`)[0] || "").split(" ");
        ...
        if ({}.hasOwnProperty.call(r.versions, "pip3"))
          if (Le) {
            ...
            k.darwinXcodeExists() || c ? F("pip3 -V 2>&1", function(u, f) {
              if (!u) {
                const d = (f.toString().split(`
`)[0] || "").split(" ");
...
`)[0] || "").split(" ");
        ...
        ({}).hasOwnProperty.call(r.versions, "java") && (Le ? F("/usr/libexec/java_home -V 2>&1", function(c, u) {
...
`)[0] || "").split('"');
          ...
          a();
        })), {}.hasOwnProperty.call(r.versions, "gcc") && (Le && k.darwinXcodeExists() || !Le ? F("gcc -dumpversion", function(c, u) {
...
`)[0].trim();
        ...
        }) : a()), {}.hasOwnProperty.call(r.versions, "virtualbox") && F(k.getVboxmanage() + " -v 2>&1", function(c, u) {
          if (!c) {
            const p = (u.toString().split(`
`)[0] || "").split("r");
...
`)[0].trim();
          ...
          a();
        }), {}.hasOwnProperty.call(r.versions, "powershell") && (ve ? k.powerShell("$PSVersionTable").then((c) => {
          const u = c.toString().toLowerCase().split(`
`).map((f) => f.replace(/ +/g, " ").replace(/ +/g, ":"));
          r.versions.powershell = k.getValue(u, "psversion"), a();
        }) : a()), {}.hasOwnProperty.call(r.versions, "dotnet") && (ve ? k.powerShell('gci "HKLM:\\SOFTWARE\\Microsoft\\NET Framework Setup\\NDP" -recurse | gp -name Version,Release -EA 0 | where { $_.PSChildName -match "^(?!S)\\p{L}"} | select PSChildName, Version, Release').then((c) => {
  ...
  return new Promise((i) => {
    process.nextTick(() => {
      if (ve)
        try {
          const e = "CMD";
          k.powerShell(`Get-CimInstance -className win32_process | where-object {$_.ProcessId -eq ${process.ppid} } | select Name`).then((n) => {
            let s = "CMD";
...
function Es(t) {
  return new Promise((i) => {
    process.nextTick(() => {
      ...
      if (Le && F("system_profiler SPHardwareDataType -json", function(s, r) {
        if (!s)
          try {
            const o = JSON.parse(r.toString());
        ...
        t && t(e), i(e);
      }), ti && F(`echo -n "os: "; cat /var/lib/dbus/machine-id 2> /dev/null ||
cat /etc/machine-id 2> /dev/null; echo;
echo -n "hardware: "; cat /sys/class/dmi/id/product_uuid 2> /dev/null; echo;`, function(r, o) {
        const a = o.toString().split(`
`);
        if (e.os = k.getValue(a, "os").toLowerCase(), e.hardware = k.getValue(a, "hardware").toLowerCase(), !e.hardware) {
          const l = Ce.readFileSync("/proc/cpuinfo", { encoding: "utf8" }).toString().split(`
`), c = k.getValue(l, "serial");
          e.hardware = c || "";
        }
        t && t(e), i(e);
      }), (Zn || er || tr) && F("sysctl -i kern.hostid kern.hostuuid", function(s, r) {
        const o = r.toString().split(`
`);
        e.os = k.getValue(o, "kern.hostid", ":").toLowerCase(), e.hardware = k.getValue(o, "kern.hostuuid", ":").toLowerCase(), e.os.indexOf("unknown") >= 0 && (e.os = ""), e.hardware.indexOf("unknown") >= 0 && (e.hardware = ""), t && t(e), i(e);
      }), ve) {
        let s = "%windir%\\System32";
        process.arch === "ia32" && Object.prototype.hasOwnProperty.call(process.env, "PROCESSOR_ARCHITEW6432") && (s = "%windir%\\sysnative\\cmd.exe /c %windir%\\System32"), k.powerShell("Get-CimInstance Win32_ComputerSystemProduct | select UUID | fl").then((r) => {
          let o = r.split(`\r
`);
          e.hardware = k.getValue(o, "uuid", ":").toLowerCase(), F(`${s}\\reg query "HKEY_LOCAL_MACHINE\\SOFTWARE\\Microsoft\\Cryptography" /v MachineGuid`, k.execOptsWin, function(a, l) {
            n = l.toString().split(`
...
ei.uuid = Es;
var Ct = {};
const Be = oe, he = te.exec, rn = te.execSync, qi = Fe, P = T;
let rt = process.platform;
const ii = rt === "linux" || rt === "android", sn = rt === "darwin", on = rt === "win32", an = rt === "freebsd", cn = rt === "openbsd", ln = rt === "netbsd", un = rt === "sunos";
let jt = 0, Y = {
  user: 0,
  nice: 0,
  system: 0,
...
function Ti(t) {
  t.brand = t.brand.replace(/\(R\)+/g, "®").replace(/\s+/g, " ").trim(), t.brand = t.brand.replace(/\(TM\)+/g, "™").replace(/\s+/g, " ").trim(), t.brand = t.brand.replace(/\(C\)+/g, "©").replace(/\s+/g, " ").trim(), t.brand = t.brand.replace(/CPU+/g, "").replace(/\s+/g, " ").trim(), t.manufacturer = Gi(t.brand);
  let i = t.brand.split(" ");
...
function Da() {
  return new Promise((t) => {
    process.nextTick(() => {
      ...
      Ms().then((n) => {
        if (e.flags = n, e.virtualization = n.indexOf("vmx") > -1 || n.indexOf("svm") > -1, sn && he("sysctl machdep.cpu hw.cpufrequency_max hw.cpufrequency_min hw.packages hw.physicalcpu_max hw.ncpu hw.tbfrequency hw.cpufamily hw.cpusubfamily", function(s, r) {
          let o = r.toString().split(`
`);
...
`)), s = P.getValue(r, "model name") || s, s = P.getValue(r, "bios model name") || s, s = P.cleanString(s);
            const l = s.split("@");
            if (e.brand = l[0].trim(), e.speed = l[1] ? parseFloat(l[1].trim()) : 0, e.speed === 0 && (e.brand.indexOf("AMD") > -1 || e.brand.toLowerCase().indexOf("ryzen") > -1) && (e.speed = Ln(e.brand)), e.speed === 0) {
              const h = $n();
            ...
            if (P.getValue(r, "architecture") === "riscv64") {
              const h = qi.readFileSync("/proc/cpuinfo").toString().split(`
`), x = P.getValue(h, "uarch") || "";
              if (x.indexOf(",") > -1) {
                const g = x.split(",");
                e.manufacturer = Gi(g[0]), e.brand = g[1];
              }
            }
            let m = [];
            he('export LC_ALL=C; dmidecode –t 4 2>/dev/null | grep "Upgrade: Socket"; unset LC_ALL', function(h, x) {
              m = x.toString().split(`
...
`);
            ...
            if (e.brand = s.split("@")[0].trim(), e.speed = s.split("@")[1] ? parseFloat(s.split("@")[1].trim()) : 0, e.speed === 0 && (e.brand.indexOf("AMD") > -1 || e.brand.toLowerCase().indexOf("ryzen") > -1) && (e.speed = Ln(e.brand)), e.speed === 0) {
              const d = $n();
              d.avg !== 0 && (e.speed = d.avg);
            }
            jt = e.speed, e.speedMin = e.speed, e.speedMax = Math.round(parseFloat(P.getValue(r, "max speed").replace(/Mhz/g, "")) / 10) / 100, e = Ti(e), e.vendor = Gi(P.getValue(r, "manufacturer"));
...
`);
              ...
              let x = parseInt(h[0], 10);
              const g = h.length > 1 ? h[1] : "kb";
              x = x * (g === "kb" ? 1024 : g === "mb" ? 1024 * 1024 : g === "gb" ? 1024 * 1024 * 1024 : 1), m && (m === "l1" ? (e.cache[m + "d"] = x / 2, e.cache[m + "i"] = x / 2) : e.cache[m] = x);
            }
        ...
        if (un && t(e), on)
          try {
            ...
            s.push(P.powerShell("Get-CimInstance Win32_processor | select Name, Revision, L2CacheSize, L3CacheSize, Manufacturer, MaxClockSpeed, Description, UpgradeMethod, Caption, NumberOfLogicalProcessors, NumberOfCores | fl")), s.push(P.powerShell("Get-CimInstance Win32_CacheMemory | select CacheType,InstalledSize,Level | fl")), s.push(P.powerShell("(Get-CimInstance Win32_ComputerSystem).HypervisorPresent")), Promise.all(
              ...
              let o = r[0].split(`\r
`), a = P.getValue(o, "name", ":") || "";
              a.indexOf("@") >= 0 ? (e.brand = a.split("@")[0].trim(), e.speed = a.split("@")[1] ? parseFloat(a.split("@")[1].trim()) : 0, jt = e.speed) : (e.brand = a.trim(), e.speed = 0), e = Ti(e), e.revision = P.getValue(o, "revision", ":"), e.vendor = P.getValue(o, "manufacturer", ":"), e.speedMax = Math.round(parseFloat(P.getValue(o, "maxclockspeed", ":").replace(/,/g, ".")) / 10) / 100, e.speed === 0 && (e.brand.indexOf("AMD") > -1 || e.brand.toLowerCase().indexOf("ryzen") > -1) && (e.speed = Ln(e.brand)), e.speed === 0 && (e.speed = e.speedMax), e.speedMin = e.speed;
              ...
              f && (e.processors = parseInt(f) || 1), d && p && (e.cores = parseInt(p) || P.cores(), e.physicalCores = parseInt(d) || P.cores()), f > 1 && (e.cores = e.cores * f, e.physicalCores = e.physicalCores * f), e.cache = Ds(r[0], r[1]);
...
function $n() {
  ...
  if (t && t.length && t[0].speed)
    ...
    try {
      const o = rn('cat /proc/cpuinfo | grep "cpu MHz" | cut -d " " -f 3', P.execOptsLinux).toString().split(`
`).filter((a) => a.length > 0);
      for (let a in o)
...
function Na(t) {
  return new Promise((i) => {
    process.nextTick(() => {
      ...
      if (ii) {
        try {
          const r = rn('cat /sys/class/thermal/thermal_zone*/type  2>/dev/null; echo "-----"; cat /sys/class/thermal/thermal_zone*/temp 2>/dev/null;', P.execOptsLinux).toString().split(`-----
`);
          if (r.length === 2) {
            const o = r[0].split(`
`), a = r[1].split(`
...
`), p = null, d = !0, m = "";
                if (f.forEach(function(h) {
                  ...
                  let x = /[+-]([^°]*)/g, g = h.match(x), y = h.split(":")[0].toUpperCase();
                  m === "acpi" ? y.indexOf("TEMP") !== -1 && e.socket.push(parseFloat(g)) : m === "pch" && y.indexOf("TEMP") !== -1 && !e.chipset && (e.chipset = parseFloat(g)), (y.indexOf("PHYSICAL") !== -1 || y.indexOf("PACKAGE") !== -1) && (e.main = parseFloat(g)), y.indexOf("CORE ") !== -1 && e.cores.push(parseFloat(g)), y.indexOf("TDIE") !== -1 && p === null && (p = parseFloat(g));
                }), e.cores.length > 0) {
                  e.main = Math.round(e.cores.reduce((x, g) => x + g, 0) / e.cores.length);
              ...
              qi.stat("/sys/class/thermal/thermal_zone0/temp", function(f) {
                f === null ? qi.readFile("/sys/class/thermal/thermal_zone0/temp", function(p, d) {
                  if (!p) {
...
`);
      ...
      if ((an || cn || ln) && he("sysctl dev.cpu | grep temp", function(n, s) {
        if (!n) {
          let r = s.toString().split(`
`), o = 0;
...
`).forEach(function(o) {
            ...
            }), e ? (t && t(e), i(e)) : qi.readFile("/proc/cpuinfo", function(r, o) {
              if (!r) {
                let a = o.toString().split(`
`);
      ...
      (an || cn || ln) && he("export LC_ALL=C; dmidecode -t 4 2>/dev/null; unset LC_ALL", function(n, s) {
        let r = [];
        if (!n) {
...
`) : []).forEach(function(l) {
      ...
      }), sn && he("sysctl machdep.cpu.features", function(n, s) {
        if (!n) {
          let r = s.toString().split(`
`);
...
`).forEach(function(o) {
      ...
      if ((an || cn || ln) && he("export LC_ALL=C; dmidecode -t 7 2>/dev/null; unset LC_ALL", function(n, s) {
        let r = [];
...
`);
        ...
        t && t(e), i(e);
      }), sn && he("sysctl hw.l1icachesize hw.l1dcachesize hw.l2cachesize hw.l3cachesize", function(n, s) {
        n || s.toString().split(`
`).forEach(function(o) {
          let a = o.split(":");
  ...
  return new Promise((t) => {
    process.nextTick(() => {
      ...
      if (Date.now() - Y.ms >= 200) {
        ...
        if (wn = r && r.length ? r.length : 0, ii)
          try {
            const g = rn("cat /proc/stat 2>/dev/null | grep cpu", P.execOptsLinux).toString().split(`
`);
            if (g.length > 1 && (g.shift(), g.length === r.length))
              for (let y = 0; y < g.length; y++) {
                ...
                if (w.length >= 10) {
                  ...
                  r[y].times.steal = S, r[y].times.guest = C;
                }
              }
          } catch {
            P.noop();
        ...
        for (let g = 0; g < wn; g++) {
          const y = r[g].times;
          o += y.user, a += y.sys, l += y.nice, u += y.idle, c += y.irq, f += y.steal || 0, p += y.guest || 0;
          let w = v && v[g] && v[g].totalTick ? v[g].totalTick : 0, S = v && v[g] && v[g].totalLoad ? v[g].totalLoad : 0, C = v && v[g] && v[g].user ? v[g].user : 0, _ = v && v[g] && v[g].sys ? v[g].sys : 0, O = v && v[g] && v[g].nice ? v[g].nice : 0, U = v && v[g] && v[g].idle ? v[g].idle : 0, b = v && v[g] && v[g].irq ? v[g].irq : 0, W = v && v[g] && v[g].steal ? v[g].steal : 0, V = v && v[g] && v[g].guest ? v[g].guest : 0;
          v[g] = y, v[g].totalTick = v[g].user + v[g].sys + v[g].nice + v[g].irq + v[g].steal + v[g].guest + v[g].idle, v[g].totalLoad = v[g].user + v[g].sys + v[g].nice + v[g].irq + v[g].steal + v[g].guest, v[g].currentTick = v[g].totalTick - w, v[g].load = v[g].totalLoad - S, v[g].loadUser = v[g].user - C, v[g].loadSystem = v[g].sys - _, v[g].loadNice = v[g].nice - O, v[g].loadIdle = v[g].idle - U, v[g].loadIrq = v[g].irq - b, v[g].loadSteal = v[g].steal - W, v[g].loadGuest = v[g].guest - V, d[g] = {}, d[g].load = v[g].load / v[g].currentTick * 100, d[g].loadUser = v[g].loadUser / v[g].currentTick * 100, d[g].loadSystem = v[g].loadSystem / v[g].currentTick * 100, d[g].loadNice = v[g].loadNice / v[g].currentTick * 100, d[g].loadIdle = v[g].loadIdle / v[g].currentTick * 100, d[g].loadIrq = v[g].loadIrq / v[g].currentTick * 100, d[g].loadSteal = v[g].loadSteal / v[g].currentTick * 100, d[g].loadGuest = v[g].loadGuest / v[g].currentTick * 100, d[g].rawLoad = v[g].load, d[g].rawLoadUser = v[g].loadUser, d[g].rawLoadSystem = v[g].loadSystem, d[g].rawLoadNice = v[g].loadNice, d[g].rawLoadIdle = v[g].loadIdle, d[g].rawLoadIrq = v[g].loadIrq, d[g].rawLoadSteal = v[g].loadSteal, d[g].rawLoadGuest = v[g].loadGuest;
        }
        let m = o + a + l + c + f + p + u, h = o + a + l + c + f + p, x = m - Y.tick;
        n = {
          avgLoad: e,
          currentLoad: (h - Y.load) / x * 100,
          currentLoadUser: (o - Y.user) / x * 100,
          currentLoadSystem: (a - Y.system) / x * 100,
          currentLoadNice: (l - Y.nice) / x * 100,
          currentLoadIdle: (u - Y.idle) / x * 100,
          currentLoadIrq: (c - Y.irq) / x * 100,
          currentLoadSteal: (f - Y.steal) / x * 100,
          currentLoadGuest: (p - Y.guest) / x * 100,
        ...
        let r = [];
        for (let o = 0; o < wn; o++)
          r[o] = {}, r[o].load = v[o].load / v[o].currentTick * 100, r[o].loadUser = v[o].loadUser / v[o].currentTick * 100, r[o].loadSystem = v[o].loadSystem / v[o].currentTick * 100, r[o].loadNice = v[o].loadNice / v[o].currentTick * 100, r[o].loadIdle = v[o].loadIdle / v[o].currentTick * 100, r[o].loadIrq = v[o].loadIrq / v[o].currentTick * 100, r[o].rawLoad = v[o].load, r[o].rawLoadUser = v[o].loadUser, r[o].rawLoadSystem = v[o].loadSystem, r[o].rawLoadNice = v[o].loadNice, r[o].rawLoadIdle = v[o].loadIdle, r[o].rawLoadIrq = v[o].loadIrq, r[o].rawLoadSteal = v[o].loadSteal, r[o].rawLoadGuest = v[o].loadGuest;
        n = {
          avgLoad: e,
...
Ct.fullLoad = Ra;
var ir = {};
const We = oe, gi = te.exec, Ui = te.execSync, M = T, Ga = Fe;
let st = process.platform;
const Bs = st === "linux" || st === "android", Vs = st === "darwin", Ns = st === "win32", ks = st === "freebsd", Fs = st === "openbsd", Ws = st === "netbsd", Rs = st === "sunos", _r = {
...
function Ua(t) {
  return new Promise((i) => {
    process.nextTick(() => {
      ...
      if (Bs)
        try {
          Ga.readFile("/proc/meminfo", function(n, s) {
            if (!n) {
...
`);
      ...
      if (Rs && (t && t(e), i(e)), Vs) {
        let n = 4096;
        try {
          n = M.toInt(Ui("sysctl -n vm.pagesize").toString()) || n;
        } catch {
          M.noop();
        }
        try {
...
function za(t) {
  function i(n) {
    return {}.hasOwnProperty.call(_r, n) ? _r[n] : n;
  }
  function e(n) {
    const s = n.replace("0x", "").toUpperCase();
    return s.length === 4 && {}.hasOwnProperty.call(Or, s) ? Or[s] : n;
  }
  return new Promise((n) => {
    process.nextTick(() => {
      let s = [];
      if ((Bs || ks || Fs || Ws) && gi('export LC_ALL=C; dmidecode -t memory 2>/dev/null | grep -iE "Size:|Type|Speed|Manufacturer|Form Factor|Locator|Memory Device|Serial Number|Voltage|Part Number"; unset LC_ALL', function(r, o) {
        if (!r) {
...
`);
        ...
        if (!s.length) {
          ...
          try {
            let a = Ui("cat /proc/cpuinfo 2>/dev/null", M.execOptsLinux), l = a.toString().split(`
`), c = M.getValue(l, "revision", ":", !0).toLowerCase();
            if (M.isRaspberry(l)) {
              ...
              s[0].type = "LPDDR2", s[0].type = c && c[2] && c[2] === "3" ? "LPDDR4" : s[0].type, s[0].type = c && c[2] && c[2] === "4" ? "LPDDR4X" : s[0].type, s[0].ecc = !1, s[0].clockSpeed = c && c[2] && u[c[2]] || 400, s[0].clockSpeed = c && c[4] && c[4] === "d" ? 500 : s[0].clockSpeed, s[0].formFactor = "SoC", a = Ui("vcgencmd get_config sdram_freq 2>/dev/null", M.execOptsLinux), l = a.toString().split(`
`);
              let f = parseInt(M.getValue(l, "sdram_freq", "=", !0), 10) || 0;
              f && (s[0].clockSpeed = f), a = Ui("vcgencmd measure_volts sdram_p 2>/dev/null", M.execOptsLinux), l = a.toString().split(`
`);
        ...
        t && t(s), n(s);
      }), Vs && gi("system_profiler SPMemoryDataType", function(r, o) {
        if (!r) {
          const a = o.toString().split(`
          ...
          M.powerShell("Get-CimInstance Win32_PhysicalMemory | select DataWidth,TotalWidth,Capacity,BankLabel,MemoryType,SMBIOSMemoryType,ConfiguredClockSpeed,Speed,FormFactor,Manufacturer,PartNumber,SerialNumber,ConfiguredVoltage,MinVoltage,MaxVoltage,Tag | fl").then((a, l) => {
            if (!l) {
              ...
              c.shift(), c.forEach(function(u) {
                let f = u.split(`\r
`);
                const p = M.toInt(M.getValue(f, "DataWidth", ":")), d = M.toInt(M.getValue(f, "TotalWidth", ":")), m = parseInt(M.getValue(f, "Capacity", ":"), 10) || 0, h = M.getValue(f, "Tag", ":"), x = M.splitByNumber(h);
                m && s.push({
                  size: m,
                  bank: M.getValue(f, "BankLabel", ":") + (x[1] ? "/" + x[1] : ""),
                  // BankLabel
...
ir.memLayout = za;
const Pr = te.exec, Wt = Fe, q = T;
let ot = process.platform;
const $a = ot === "linux" || ot === "android", Ha = ot === "darwin", qa = ot === "win32", ja = ot === "freebsd", Xa = ot === "openbsd", Ka = ot === "netbsd", Ya = ot === "sunos";
function Ja(t, i, e) {
  const n = {};
...
var Qa = function(t) {
  return new Promise((i) => {
    process.nextTick(() => {
      ...
      if ($a) {
        let n = "";
        Wt.existsSync("/sys/class/power_supply/BAT1/uevent") ? n = "/sys/class/power_supply/BAT1/" : Wt.existsSync("/sys/class/power_supply/BAT0/uevent") && (n = "/sys/class/power_supply/BAT0/");
        let s = !1, r = "";
        Wt.existsSync("/sys/class/power_supply/AC/online") ? r = "/sys/class/power_supply/AC/online" : Wt.existsSync("/sys/class/power_supply/AC0/online") && (r = "/sys/class/power_supply/AC0/online"), r && (s = Wt.readFileSync(r).toString().trim() === "1"), n ? Wt.readFile(n + "uevent", function(o, a) {
          if (o)
            t && t(e), i(e);
          else {
            let l = a.toString().split(`
`);
      ...
      if ((ja || Xa || Ka) && Pr("sysctl -i hw.acpi.battery hw.acpi.acline", function(n, s) {
        let r = s.toString().split(`
`);
      ...
      }), Ha && Pr('ioreg -n AppleSmartBattery -r | egrep "CycleCount|IsCharging|DesignCapacity|MaxCapacity|CurrentCapacity|DeviceName|BatterySerialNumber|Serial|TimeRemaining|Voltage"; pmset -g batt | grep %', function(n, s) {
        if (s) {
...
`).filter((u) => u);
              if (o.length) {
                ...
                for (let p = 0; p < o.length; p++) {
                  let d = o[p][0].split(`\r
`);
                  const m = l && l.length >= p + 1 && l[p] ? q.toInt(l[p]) : 0, h = c && c.length >= p + 1 && c[p] ? q.toInt(c[p]) : 0, x = Ja(d, m, h);
                  !u && x.status > 0 && x.status !== 10 ? (e.hasBattery = x.hasBattery, e.maxCapacity = x.maxCapacity, e.designedCapacity = x.designedCapacity, e.voltage = x.voltage, e.capacityUnit = x.capacityUnit, e.percent = x.percent, e.currentCapacity = x.currentCapacity, e.isCharging = x.isCharging, e.acConnected = x.acConnected, e.model = x.model, u = !0) : x.status !== -1 && f.push(
                    {
                      hasBattery: x.hasBattery,
...
const Di = Fe, Rt = te.exec, vn = te.execSync, D = T;
let at = process.platform, ci = "";
const Bi = at === "linux" || at === "android", Za = at === "darwin", Ar = at === "win32", ec = at === "freebsd", tc = at === "openbsd", ic = at === "netbsd", nc = at === "sunos";
let li = 0, ui = 0, Vi = 0, Ni = 0;
const Er = {
  "-2": "UNINITIALIZED",
  "-1": "OTHER",
  0: "HD15",
...
function ac(t) {
  function i(p) {
    ...
    try {
      return p.forEach(function(m) {
        const h = (m.sppci_bus || "").indexOf("builtin") > -1 ? "Built-In" : (m.sppci_bus || "").indexOf("pcie") > -1 ? "PCIe" : "", x = (parseInt(m.spdisplays_vram || "", 10) || 0) * ((m.spdisplays_vram || "").indexOf("GB") > -1 ? 1024 : 1), g = (parseInt(m.spdisplays_vram_shared || "", 10) || 0) * ((m.spdisplays_vram_shared || "").indexOf("GB") > -1 ? 1024 : 1);
        let y = oc(m.spdisplays_metal || m.spdisplays_metalfamily || "");
        d.controllers.push({
          vendor: Mr(m.spdisplays_vendor || "") || m.spdisplays_vendor || "",
          model: m.sppci_model || "",
          bus: h,
          vramDynamic: h === "Built-In",
          vram: x || g || null,
          deviceId: m["spdisplays_device-id"] || "",
          vendorId: m["spdisplays_vendor-id"] || sc((m.spdisplays_vendor || "") + (m.sppci_model || "")),
          ...
          const S = w.spdisplays_connection_type || "", C = (w._spdisplays_resolution || "").split("@"), _ = C[0].split("x"), O = (w._spdisplays_pixels || "").split("x"), U = w.spdisplays_depth || "", b = w["_spdisplays_display-serial-number"] || w["_spdisplays_display-serial-number2"] || null;
          d.displays.push({
            vendor: rc(w["_spdisplays_display-vendor-id"] || "") || Mr(w._name || ""),
  ...
  function e(p) {
    ...
    try {
      x = vn('export LC_ALL=C; dmidecode -t 9 2>/dev/null; unset LC_ALL | grep "Bus Address: "', D.execOptsLinux).toString().split(`
`);
      for (let y = 0; y < x.length; y++)
        x[y] = x[y].replace("Bus Address:", "").replace("0000:", "").trim();
    ...
    return p.forEach((y) => {
      let w = "";
      if (g < p.length && p[g] && (w = p[g], w.indexOf(":") > 0 && (w = w.split(":")[1])), y.trim() !== "") {
        if (y[0] !== " " && y[0] !== "	") {
          let S = x.indexOf(y.split(" ")[0]) >= 0, C = y.toLowerCase().indexOf(" vga "), _ = y.toLowerCase().indexOf("3d controller");
          if (C !== -1 || _ !== -1) {
            ...
            let U = y.search(/\[[0-9a-f]{4}:[0-9a-f]{4}]|$/), b = y.substr(C, U - C).split(":");
            if (m.busAddress = y.substr(0, C).trim(), b.length > 1 && (b[1] = b[1].trim(), b[1].toLowerCase().indexOf("corporation") >= 0 ? (m.vendor = b[1].substr(0, b[1].toLowerCase().indexOf("corporation") + 11).trim(), m.model = b[1].substr(b[1].toLowerCase().indexOf("corporation") + 11, 200).split("(")[0].trim(), m.bus = x.length > 0 && S ? "PCIe" : "Onboard", m.vram = null, m.vramDynamic = !1) : b[1].toLowerCase().indexOf(" inc.") >= 0 ? ((b[1].match(/]/g) || []).length > 1 ? (m.vendor = b[1].substr(0, b[1].toLowerCase().indexOf("]") + 1).trim(), m.model = b[1].substr(b[1].toLowerCase().indexOf("]") + 1, 200).trim().split("(")[0].trim()) : (m.vendor = b[1].substr(0, b[1].toLowerCase().indexOf(" inc.") + 5).trim(), m.model = b[1].substr(b[1].toLowerCase().indexOf(" inc.") + 5, 200).trim().split("(")[0].trim()), m.bus = x.length > 0 && S ? "PCIe" : "Onboard", m.vram = null, m.vramDynamic = !1) : b[1].toLowerCase().indexOf(" ltd.") >= 0 && ((b[1].match(/]/g) || []).length > 1 ? (m.vendor = b[1].substr(0, b[1].toLowerCase().indexOf("]") + 1).trim(), m.model = b[1].substr(b[1].toLowerCase().indexOf("]") + 1, 200).trim().split("(")[0].trim()) : (m.vendor = b[1].substr(0, b[1].toLowerCase().indexOf(" ltd.") + 5).trim(), m.model = b[1].substr(b[1].toLowerCase().indexOf(" ltd.") + 5, 200).trim().split("(")[0].trim())), m.model && w.indexOf(m.model) !== -1)) {
  ...
  function n(p, d) {
    const m = /\[([^\]]+)\]\s+(\w+)\s+(.*)/, h = d.reduce((x, g) => {
      const y = m.exec(g.trim());
      return y && (x[y[1]] || (x[y[1]] = {}), x[y[1]][y[2]] = y[3]), x;
    }, {});
    for (let x in h) {
      const g = h[x];
      if (g.CL_DEVICE_TYPE === "CL_DEVICE_TYPE_GPU") {
        ...
        if (g.CL_DEVICE_TOPOLOGY_AMD) {
          ...
          if (!isNaN(w) && !isNaN(S)) {
            ...
            y = `${C.toString().padStart(2, "0")}:${_.toString().padStart(2, "0")}.${O}`;
          }
        }
        if (y) {
          let w = p.find((C) => C.busAddress === y);
          w || (w = {
            vendor: "",
            model: "",
            bus: "",
  ...
  function s() {
    ...
    if (Ar)
      try {
        const p = D.WINDIR + "\\System32\\DriverStore\\FileRepository", m = Di.readdirSync(p).filter((h) => Di.readdirSync([p, h].join("/")).includes("nvidia-smi.exe")).reduce((h, x) => {
          const g = Di.statSync([p, h, "nvidia-smi.exe"].join("/")), y = Di.statSync([p, x, "nvidia-smi.exe"].join("/"));
          return g.ctimeMs > y.ctimeMs ? h : x;
        });
  ...
  function o() {
    function p(x) {
      return [null, void 0].includes(x) ? x : parseFloat(x);
    }
    const d = r();
    if (!d)
      return [];
...
`).filter(Boolean).map((x) => {
      const g = x.split(", ").map((y) => y.includes("N/A") ? void 0 : y);
      return g.length === 16 ? {
        driverVersion: g[0],
        subDeviceId: g[1],
        name: g[2],
    ...
    return h = h.filter((x) => "pciBus" in x), h;
  }
  function a(p, d) {
  ...
  function l(p) {
    ...
    if (p.substr(m, 6) === "000000" && (m += 36), p.substr(m, 6) === "000000" && (m += 36), p.substr(m, 6) === "000000" && (m += 36), p.substr(m, 6) === "000000" && (m += 36), d.resolutionX = parseInt("0x0" + p.substr(m + 8, 1) + p.substr(m + 4, 2)), d.resolutionY = parseInt("0x0" + p.substr(m + 14, 1) + p.substr(m + 10, 2)), d.sizeX = parseInt("0x0" + p.substr(m + 28, 1) + p.substr(m + 24, 2)), d.sizeY = parseInt("0x0" + p.substr(m + 29, 1) + p.substr(m + 26, 2)), m = p.indexOf("000000fc00"), m >= 0) {
      ...
      try {
        h.length > 2 && (d.model = h.match(/.{1,2}/g).map(function(x) {
  ...
  function c(p, d) {
    ...
    for (let S = 1; S < p.length; S++)
      if (p[S].trim() !== "") {
        ...
        if (x)
          if (p[S].search(/\S|$/) > w)
            y += p[S].toLowerCase().trim();
          else {
            let C = l(y);
            h.vendor = C.vendor, h.model = C.model, h.resolutionX = C.resolutionX, h.resolutionY = C.resolutionY, h.sizeX = C.sizeX, h.sizeY = C.sizeY, h.pixelDepth = d, x = !1;
          }
        if (p[S].toLowerCase().indexOf("edid:") >= 0 && (x = !0, w = p[S].search(/\S|$/)), p[S].toLowerCase().indexOf("*current") >= 0) {
          const C = p[S].split("(");
  ...
  return new Promise((p) => {
    process.nextTick(() => {
      ...
      if (Za && Rt("system_profiler -xml -detailLevel full SPDisplaysDataType", function(h, x) {
        if (!h) {
          try {
            const g = x.toString();
          ...
          try {
            x = vn('defaults read /Library/Preferences/com.apple.windowserver.plist 2>/dev/null;defaults read /Library/Preferences/com.apple.windowserver.displays.plist 2>/dev/null; echo ""', { maxBuffer: 1024 * 2e4 });
            const g = (x || "").toString(), y = D.plistReader(g);
            if (y.DisplayAnyUserSets && y.DisplayAnyUserSets.Configs && y.DisplayAnyUserSets.Configs[0] && y.DisplayAnyUserSets.Configs[0].DisplayConfig) {
              const w = y.DisplayAnyUserSets.Configs[0].DisplayConfig;
              let S = 0;
              w.forEach((C) => {
            ...
            if (y.DisplayAnyUserSets && y.DisplayAnyUserSets.length > 0 && y.DisplayAnyUserSets[0].length > 0 && y.DisplayAnyUserSets[0][0].DisplayID) {
              const w = y.DisplayAnyUserSets[0];
              let S = 0;
              w.forEach((C) => {
        ...
        t && t(d), p(d);
      }), Bi && (D.isRaspberry() && Rt(`fbset -s 2> /dev/null | grep 'mode "' ; vcgencmd get_mem gpu 2> /dev/null; tvservice -s 2> /dev/null; tvservice -n 2> /dev/null;`, function(x, g) {
        let y = g.toString().split(`
`);
        if (y.length > 3 && y[0].indexOf('mode "') >= -1 && y[2].indexOf("0x12000a") > -1) {
          const w = y[0].replace("mode", "").replace(/"/g, "").trim().split("x");
          w.length === 2 && d.displays.push({
            vendor: "",
            model: D.getValue(y, "device_name", "="),
            main: !0,
            builtin: !1,
            connection: "HDMI",
            sizeX: null,
        ...
        y.length >= 1 && g.toString().indexOf("gpu=") >= -1 && d.controllers.push({
          vendor: "Broadcom",
          model: D.getRpiGpu(),
          bus: "",
          vram: D.getValue(y, "gpu", "=").replace("M", ""),
          vramDynamic: !0
        });
      }), Rt("lspci -vvv  2>/dev/null", function(h, x) {
        if (!h) {
...
`);
        ...
        Rt("clinfo --raw", function(y, w) {
          if (!y) {
            let C = w.toString().split(`
`);
            d.controllers = n(d.controllers, C);
          }
          Rt("xdpyinfo 2>/dev/null | grep 'depth of root window' | awk '{ print $5 }'", function(C, _) {
            let O = 0;
            if (!C) {
              let b = _.toString().split(`
`);
              O = parseInt(b[0]) || 0;
            }
            Rt("xrandr --verbose 2>/dev/null", function(b, W) {
              if (!b) {
...
`);
      ...
      })), (ec || tc || ic) && (t && t(null), p(null)), nc && (t && t(null), p(null)), Ar)
        try {
          const m = [];
          ...
          Promise.all(
            ...
            let g = x[0].replace(/\r/g, "").split(/\n\s*\n/), y = x[1].replace(/\r/g, "").split(/\n\s*\n/);
            d.controllers = u(g, y), d.controllers = d.controllers.map((b) => b.vendor.toLowerCase() === "nvidia" ? a(b, h.find((W) => {
              let V = (b.subDeviceId || "").toLowerCase();
              const ie = W.subDeviceId.split("x");
              let se = ie.length > 1 ? ie[1].toLowerCase() : ie[0].toLowerCase();
              const le = Math.abs(V.length - se.length);
              if (V.length > se.length)
                for (let X = 0; X < le; X++)
                  se = "0" + se;
              else if (V.length < se.length)
                for (let X = 0; X < le; X++)
                  V = "0" + V;
              return V === se;
            }) || {}) : b);
            let w = x[2].replace(/\r/g, "").split(/\n\s*\n/);
            w[0].trim() === "" && w.shift(), w.length && w[w.length - 1].trim() === "" && w.pop();
            let S = x[3].replace(/\r/g, "").split("Active ");
            S.shift();
            let C = x[4].replace(/\r/g, "").split("BitsPerPixel ");
            C.shift();
            let _ = x[5].replace(/\r/g, "").split(/\n\s*\n/);
            _.shift();
            const O = x[6].replace(/\r/g, "").split(/\n/);
            let U = [];
            O.forEach((b) => {
              const W = b.split("|");
              W.length === 5 && U.push({
            ...
            }), d.displays = f(C, S, w, _, U), d.displays.length === 1 && (li && (d.displays[0].resolutionX = li, d.displays[0].currentResX || (d.displays[0].currentResX = li)), ui && (d.displays[0].resolutionY = ui, d.displays[0].currentResY === 0 && (d.displays[0].currentResY = ui)), Vi && (d.displays[0].pixelDepth = Vi)), d.displays = d.displays.map((b) => (Ni && !b.currentRefreshRate && (b.currentRefreshRate = Ni), b)), t && t(d), p(d);
          }).catch(() => {
            t && t(d), p(d);
          });
        } catch {
  ...
  function u(p, d) {
    const m = {};
    for (const x in d)
      if ({}.hasOwnProperty.call(d, x) && d[x].trim() !== "") {
        const g = d[x].trim().split(`
`), y = D.getValue(g, "MatchingDeviceId").match(/PCI\\(VEN_[0-9A-F]{4})&(DEV_[0-9A-F]{4})(?:&(SUBSYS_[0-9A-F]{8}))?(?:&(REV_[0-9A-F]{2}))?/i);
        if (y) {
          const w = parseInt(D.getValue(g, "HardwareInformation.qwMemorySize"));
          if (!isNaN(w)) {
            let S = y[1].toUpperCase() + "&" + y[2].toUpperCase();
            y[3] && (S += "&" + y[3].toUpperCase()), y[4] && (S += "&" + y[4].toUpperCase()), m[S] = w;
          }
        }
      }
    let h = [];
    for (let x in p)
      if ({}.hasOwnProperty.call(p, x) && p[x].trim() !== "") {
        let g = p[x].trim().split(`
`), y = D.getValue(g, "PNPDeviceID", ":").match(/PCI\\(VEN_[0-9A-F]{4})&(DEV_[0-9A-F]{4})(?:&(SUBSYS_[0-9A-F]{8}))?(?:&(REV_[0-9A-F]{2}))?/i), w = null, S = null;
        if (y) {
          if (w = y[3] || "", w && (w = w.split("_")[1]), S == null && y[3] && y[4]) {
            const C = y[1].toUpperCase() + "&" + y[2].toUpperCase() + "&" + y[3].toUpperCase() + "&" + y[4].toUpperCase();
            ({}).hasOwnProperty.call(m, C) && (S = m[C]);
          }
          if (S == null && y[3]) {
            const C = y[1].toUpperCase() + "&" + y[2].toUpperCase() + "&" + y[3].toUpperCase();
            ({}).hasOwnProperty.call(m, C) && (S = m[C]);
          }
          if (S == null && y[4]) {
            const C = y[1].toUpperCase() + "&" + y[2].toUpperCase() + "&" + y[4].toUpperCase();
            ({}).hasOwnProperty.call(m, C) && (S = m[C]);
          }
          if (S == null) {
            const C = y[1].toUpperCase() + "&" + y[2].toUpperCase();
            ({}).hasOwnProperty.call(m, C) && (S = m[C]);
          }
        }
        h.push({
          ...
          subDeviceId: w
        }), li = D.toInt(D.getValue(g, "CurrentHorizontalResolution", ":")) || li, ui = D.toInt(D.getValue(g, "CurrentVerticalResolution", ":")) || ui, Ni = D.toInt(D.getValue(g, "CurrentRefreshRate", ":")) || Ni, Vi = D.toInt(D.getValue(g, "CurrentBitsPerPixel", ":")) || Vi;
      }
    return h;
  }
  function f(p, d, m, h, x) {
    let g = [], y = "", w = "", S = "", C = 0, _ = 0;
    if (m && m.length) {
      let O = m[0].split(`
`);
      y = D.getValue(O, "MonitorManufacturer", ":"), w = D.getValue(O, "Name", ":"), S = D.getValue(O, "PNPDeviceID", ":").replace(/&amp;/g, "&").toLowerCase(), C = D.toInt(D.getValue(O, "ScreenWidth", ":")), _ = D.toInt(D.getValue(O, "ScreenHeight", ":"));
    }
...
`), W = h[O].split(`
`);
        const V = D.getValue(U, "BitsPerPixel"), ie = D.getValue(U, "Bounds").replace("{", "").replace("}", "").replace(/=/g, ":").split(","), se = D.getValue(U, "Primary"), le = D.getValue(b, "MaxHorizontalImageSize"), X = D.getValue(b, "MaxVerticalImageSize"), fe = D.getValue(b, "InstanceName").toLowerCase(), $ = D.getValue(W, "VideoOutputTechnology"), Z = D.getValue(U, "DeviceName");
        let H = "", z = "";
        x.forEach((B) => {
          B.instanceId.toLowerCase().startsWith(fe) && y.startsWith("(") && w.startsWith("PnP") && (H = B.vendor, z = B.model);
        }), g.push({
          vendor: fe.startsWith(S) && H === "" ? y : H,
          model: fe.startsWith(S) && z === "" ? w : z,
          deviceName: Z,
          main: se.toLowerCase() === "true",
          builtin: $ === "2147483648",
          connection: $ && Er[$] ? Er[$] : "",
...
Gs.graphics = ac;
var Bt = {};
const I = T, Tr = Fe, me = te.exec, et = te.execSync, cc = I.promisifySave(te.exec);
let ct = process.platform;
const Ie = ct === "linux" || ct === "android", it = ct === "darwin", ni = ct === "win32", be = ct === "freebsd", _e = ct === "openbsd", Oe = ct === "netbsd", ri = ct === "sunos";
let ee = {}, R = {};
function lc(t, i) {
  I.isFunction(t) && (i = t, t = "");
  let e = [], n = [];
  function s(l) {
    if (!l.startsWith("/"))
      return "NFS";
...
`);
  ...
  function a(l) {
    ...
    return l.forEach(function(u) {
      if (u !== "" && (u = u.replace(/ +/g, " ").split(" "), u && (u[0].startsWith("/") || u[6] && u[6] === "/" || u[0].indexOf("/") > 0 || u[0].indexOf(":") === 1 || !it && !r(u[1])))) {
        const f = u[0], p = Ie || be || _e || Oe ? u[1] : s(u[0]), d = parseInt(Ie || be || _e || Oe ? u[2] : u[1]) * 1024, m = parseInt(Ie || be || _e || Oe ? u[3] : u[2]) * 1024, h = parseInt(Ie || be || _e || Oe ? u[4] : u[3]) * 1024, x = parseFloat((100 * (m / (m + h))).toFixed(2));
        let g = n && Object.keys(n).length > 0 ? n[f] || !1 : null;
        u.splice(0, Ie || be || _e || Oe ? 6 : 5);
        const y = u.join(" ");
...
`).filter((f) => f.startsWith("/")).forEach((f) => {
        ...
        if (Ie)
          try {
            u = "export LC_ALL=C; df -lkPTx squashfs; unset LC_ALL", et("cat /proc/mounts 2>/dev/null", I.execOptsLinux).toString().split(`
`).filter((f) => f.startsWith("/")).forEach((f) => {
...
`).forEach((f) => {
      ...
      if (ri && (i && i(c), l(c)), ni)
        try {
          ...
          I.powerShell(u).then((f, p) => {
            p || f.toString().split(/\n\s*\n/).forEach(function(m) {
              let h = m.split(`\r
`);
              const x = I.toInt(I.getValue(h, "size", ":")), g = I.toInt(I.getValue(h, "freespace", ":")), y = I.getValue(h, "caption", ":"), w = I.getValue(h, "access", ":"), S = w ? I.toInt(w) !== 1 : null;
              x && c.push({
                fs: y,
                type: I.getValue(h, "filesystem", ":"),
                size: x,
                used: x - g,
                available: g,
                use: parseFloat((100 * (x - g) / x).toFixed(2)),
                mount: y,
                rw: S
              });
            }), i && i(c), l(c);
...
function uc(t) {
  return new Promise((i) => {
    process.nextTick(() => {
      ...
      (be || _e || Oe || it) && me("sysctl -i kern.maxfiles kern.num_files kern.open_files", { maxBuffer: 1024 * 1024 }, function(s, r) {
        if (!s) {
          let o = r.toString().split(`
`);
          e.max = parseInt(I.getValue(o, "kern.maxfiles", ":"), 10), e.allocated = parseInt(I.getValue(o, "kern.num_files", ":"), 10) || parseInt(I.getValue(o, "kern.open_files", ":"), 10), e.available = e.max - e.allocated;
        }
        t && t(e), i(e);
      }), Ie && Tr.readFile("/proc/sys/fs/file-nr", function(n, s) {
        if (n)
          Tr.readFile("/proc/sys/fs/file-max", function(r, o) {
            if (!r) {
              let a = o.toString().split(`
`);
...
`);
      ...
      }), ri && (t && t(null), i(null)), ni && (t && t(null), i(null));
    });
  });
}
Bt.fsOpenFiles = uc;
...
function Hn(t) {
  ...
  }), i = I.unique(i), i = I.sortByKey(i, ["type", "name"]), i;
}
function dc(t) {
  const i = I.getValue(t, "md_level", "="), e = I.getValue(t, "md_name", "="), n = I.getValue(t, "md_uuid", "="), s = [];
  return t.forEach((r) => {
  ...
  try {
    t.forEach((e) => {
      if (e.type.startsWith("raid")) {
        const n = et(`mdadm --export --detail /dev/${e.name}`, I.execOptsLinux).toString().split(`
`), s = dc(n);
...
function wc(t) {
  return new Promise((i) => {
    process.nextTick(() => {
      let e = [];
      if (Ie && me("lsblk -bPo NAME,TYPE,SIZE,FSTYPE,MOUNTPOINT,UUID,ROTA,RO,RM,TRAN,SERIAL,LABEL,MODEL,OWNER 2>/dev/null", { maxBuffer: 1024 * 1024 }, function(n, s) {
        if (n)
...
`);
          ...
          let r = qn(s).split(`
`);
          e = Hn(r), e = Dr(e), e = hc(e), t && t(e), i(e);
        }
      }), it && me("diskutil info -all", { maxBuffer: 1024 * 1024 }, function(n, s) {
        if (!n) {
          let r = s.toString().split(`
`);
          e = fc(r), e = xc(e);
        ...
        try {
          const s = [];
          s.push(I.powerShell("Get-CimInstance -ClassName Win32_LogicalDisk | select Caption,DriveType,Name,FileSystem,Size,VolumeSerialNumber,VolumeName | fl")), s.push(I.powerShell("Get-WmiObject -Class Win32_diskdrive | Select-Object -Property PNPDeviceId,DeviceID, Model, Size, @{L='Partitions'; E={$_.GetRelated('Win32_DiskPartition').GetRelated('Win32_LogicalDisk') | Select-Object -Property DeviceID, VolumeName, Size, FreeSpace}} | fl")), I.promiseAll(
            s
          ).then((r) => {
            let o = r.results[0].toString().split(/\n\s*\n/), a = r.results[1].toString().split(/\n\s*\n/);
            o.forEach(function(l) {
              let c = l.split(`\r
`), u = I.getValue(c, "drivetype", ":");
              u && e.push({
                name: I.getValue(c, "name", ":"),
                identifier: I.getValue(c, "caption", ":"),
                type: "disk",
                fsType: I.getValue(c, "filesystem", ":").toLowerCase(),
                mount: I.getValue(c, "caption", ":"),
                size: I.getValue(c, "size", ":"),
                physical: u >= 0 && u <= 6 ? n[u] : n[0],
                uuid: I.getValue(c, "volumeserialnumber", ":"),
                label: I.getValue(c, "volumename", ":"),
                model: "",
                serial: I.getValue(c, "volumeserialnumber", ":"),
                removable: u === "2",
                protocol: "",
                group: "",
                device: ""
...
function Cc(t) {
  return new Promise((i) => {
    process.nextTick(() => {
      ...
      ee && !ee.ms || ee && ee.ms && Date.now() - ee.ms >= 500 ? (Ie && me("lsblk -r 2>/dev/null | grep /", { maxBuffer: 1024 * 1024 }, function(r, o) {
        if (r)
          t && t(e), i(e);
        else {
          let a = o.toString().split(`
...
`).forEach(function(d) {
      ...
      }), it && me(`ioreg -c IOBlockStorageDriver -k Statistics -r -w0 | sed -n "/IOBlockStorageDriver/,/Statistics/p" | grep "Statistics" | tr -cd "01234567890,
...
function Lc(t) {
  return new Promise((i) => {
    process.nextTick(() => {
      ...
      R && !R.ms || R && R.ms && Date.now() - R.ms >= 500 ? ((Ie || be || _e || Oe) && me('for mount in `lsblk 2>/dev/null | grep " disk " | sed "s/[│└─├]//g" | awk \'{$1=$1};1\' | cut -d " " -f 1 | sort -u`; do cat /sys/block/$mount/stat | sed -r "s/ +/;/g" | sed -r "s/^;//"; done', { maxBuffer: 1024 * 1024 }, function(c, u) {
        c ? (t && t(e), i(e)) : (u.split(`
`).forEach(function(p) {
          if (!p)
            return;
          ...
          if (!a)
            try {
              ...
              try {
                ...
                try {
                  const p = et("export LC_ALL=C; lsblk -bPo NAME,TYPE,SIZE,FSTYPE,MOUNTPOINT,UUID,ROTA,RO,RM,LABEL,MODEL,OWNER,GROUP 2>/dev/null; unset LC_ALL", I.execOptsLinux).toString();
                  let d = qn(p).split(`
`);
                  u = Hn(d).filter((h) => h.type === "disk" && h.size > 0 && (h.model !== null && h.model !== "" || h.mount === "" && h.label === "" && h.fsType === ""));
                } catch {
                  I.noop();
                }
              }
              u.forEach((f) => {
                ...
                try {
                  p = et("cat /sys/block/" + m + "/queue/rotational 2>/dev/null", I.execOptsLinux).toString().split(`
`)[0];
                } catch {
                  I.noop();
                }
...
`).forEach((h) => {
      ...
      if ((be || _e || Oe) && (t && t(s), e(s)), ri && (t && t(s), e(s)), it && me("system_profiler SPSerialATADataType SPNVMeDataType SPUSBDataType", { maxBuffer: 1024 * 1024 }, function(o, a) {
        if (!o) {
          let l = a.toString().split(`
`), c = [], u = [], f = [], p = "SATA";
          l.forEach((d) => {
...
`).split(" Physical Interconnect: ");
            d.shift(), d.forEach(function(m) {
              ...
              let h = m.split(`
`);
              const x = I.getValue(h, "Medium Type", ":", !0).trim(), g = I.getValue(h, "capacity", ":", !0).trim(), y = I.getValue(h, "BSD Name", ":", !0).trim();
              if (g) {
                let w = 0;
                if (g.indexOf("(") >= 0 && (w = parseInt(g.match(/\(([^)]+)\)/)[1].replace(/\./g, "").replace(/,/g, "").replace(/\s/g, ""))), w || (w = parseInt(g)), w) {
                  const S = I.getValue(h, "S.M.A.R.T. status", ":", !0).trim().toLowerCase();
                  s.push({
                    device: y,
                    type: x.startsWith("Solid") ? "SSD" : "HD",
                    name: I.getValue(h, "Model", ":", !0).trim(),
                    vendor: i(I.getValue(h, "Model", ":", !0).trim()) || I.getValue(h, "Manufacturer", ":", !0),
                    size: w,
                    bytesPerSector: null,
                    totalCylinders: null,
                    totalHeads: null,
                    ...
                    tracksPerCylinder: null,
                    sectorsPerTrack: null,
                    firmwareRevision: I.getValue(h, "Revision", ":", !0).trim(),
                    serialNum: I.getValue(h, "Serial Number", ":", !0).trim(),
                    interfaceType: I.getValue(h, "InterfaceType", ":", !0).trim(),
...
`).split(`
          ...
          Capacity:`);
            d.shift(), d.forEach(function(m) {
              ...
              let h = m.split(`
`);
              const x = I.getValue(h, "link width", ":", !0).trim(), g = I.getValue(h, "!capacity", ":", !0).trim(), y = I.getValue(h, "BSD Name", ":", !0).trim();
              if (g) {
                let w = 0;
                if (g.indexOf("(") >= 0 && (w = parseInt(g.match(/\(([^)]+)\)/)[1].replace(/\./g, "").replace(/,/g, "").replace(/\s/g, ""))), w || (w = parseInt(g)), w) {
                  const S = I.getValue(h, "S.M.A.R.T. status", ":", !0).trim().toLowerCase();
                  s.push({
                    ...
                    type: "NVMe",
                    name: I.getValue(h, "Model", ":", !0).trim(),
                    vendor: i(I.getValue(h, "Model", ":", !0).trim()),
                    size: w,
                    bytesPerSector: null,
                    totalCylinders: null,
                    totalHeads: null,
 ...
 `, "Model:").split(`
          ...
          Product ID:`);
            d.shift(), d.forEach(function(m) {
              let h = m.split(`
`);
              const x = I.getValue(h, "Capacity", ":", !0).trim(), g = I.getValue(h, "BSD Name", ":", !0).trim();
              if (x) {
                let y = 0;
                if (x.indexOf("(") >= 0 && (y = parseInt(x.match(/\(([^)]+)\)/)[1].replace(/\./g, "").replace(/,/g, "").replace(/\s/g, ""))), y || (y = parseInt(x)), y) {
                  const w = I.getValue(h, "S.M.A.R.T. status", ":", !0).trim().toLowerCase();
                  s.push({
                    device: g,
                    type: "USB",
                    name: I.getValue(h, "Model", ":", !0).trim().replaceAll(":", ""),
                    vendor: i(I.getValue(h, "Model", ":", !0).trim()),
                    size: y,
                    bytesPerSector: null,
                    totalCylinders: null,
...
`).forEach((x) => {
                  ...
                  if (g.length === 2) {
                    ...
                    if (w.length === 2) {
                      ...
                      for (let C = 0; C < s.length; C++)
                        s[C].BSDName === y && (s[C].smartStatus = S === "not supported" ? "not supported" : S === "verified" ? "Ok" : S === "failing" ? "Predicted Failure" : "unknown");
                    }
                  }
                }
              });
        ...
        try {
          const o = [];
          if (o.push(I.powerShell("Get-CimInstance Win32_DiskDrive | select Caption,Size,Status,PNPDeviceId,DeviceId,BytesPerSector,TotalCylinders,TotalHeads,TotalSectors,TotalTracks,TracksPerCylinder,SectorsPerTrack,FirmwareRevision,SerialNumber,InterfaceType | fl")), o.push(I.powerShell("Get-PhysicalDisk | select BusType,MediaType,FriendlyName,Model,SerialNumber,Size | fl")), I.smartMonToolsInstalled())
            try {
              const a = JSON.parse(et("smartctl --scan -j").toString());
              a && a.devices && a.devices.length > 0 && a.devices.forEach((l) => {
                o.push(cc(`smartctl -j -a ${l.name}`, I.execOptsWin));
              });
            } catch {
              I.noop();
            }
          I.promiseAll(
            ...
            l.forEach(function(c) {
              let u = c.split(`\r
`);
              const f = I.getValue(u, "Size", ":").trim(), p = I.getValue(u, "Status", ":").trim().toLowerCase();
              f && s.push({
                device: I.getValue(u, "DeviceId", ":"),
                ...
                name: I.getValue(u, "Caption", ":"),
                vendor: i(I.getValue(u, "Caption", ":", !0).trim()),
                size: parseInt(f),
                bytesPerSector: parseInt(I.getValue(u, "BytesPerSector", ":")),
                totalCylinders: parseInt(I.getValue(u, "TotalCylinders", ":")),
                totalHeads: parseInt(I.getValue(u, "TotalHeads", ":")),
                totalSectors: parseInt(I.getValue(u, "TotalSectors", ":")),
                totalTracks: parseInt(I.getValue(u, "TotalTracks", ":")),
                tracksPerCylinder: parseInt(I.getValue(u, "TracksPerCylinder", ":")),
                sectorsPerTrack: parseInt(I.getValue(u, "SectorsPerTrack", ":")),
                firmwareRevision: I.getValue(u, "FirmwareRevision", ":").trim(),
                serialNum: I.getValue(u, "SerialNumber", ":").trim(),
                interfaceType: I.getValue(u, "InterfaceType", ":").trim(),
              ...
              let u = c.split(`\r
`);
              const f = I.getValue(u, "SerialNumber", ":").trim(), p = I.getValue(u, "FriendlyName", ":").trim().replace("Msft ", "Microsoft"), d = I.getValue(u, "Size", ":").trim(), m = I.getValue(u, "Model", ":").trim(), h = I.getValue(u, "BusType", ":").trim();
              let x = I.getValue(u, "MediaType", ":").trim();
              if ((x === "3" || x === "HDD") && (x = "HD"), x === "4" && (x = "SSD"), x === "5" && (x = "SCM"), x === "Unspecified" && (m.toLowerCase().indexOf("virtual") > -1 || m.toLowerCase().indexOf("vbox") > -1) && (x = "Virtual"), d) {
                let g = I.findObjectByKey(s, "serialNum", f);
                (g === -1 || f === "") && (g = I.findObjectByKey(s, "name", p)), g != -1 && (s[g].type = x, s[g].interfaceType = h);
              }
              ...
              try {
                ...
                if (u.serial_number) {
                  ...
                  let p = I.findObjectByKey(s, "serialNum", f);
...
Bt.diskLayout = vc;
var Vt = {};
const ji = oe, we = te.exec, ce = te.execSync, Ic = Fe, E = T;
let lt = process.platform;
const He = lt === "linux" || lt === "android", qe = lt === "darwin", Ai = lt === "win32", ut = lt === "freebsd", pt = lt === "openbsd", ft = lt === "netbsd", Nr = lt === "sunos";
let Q = {}, kr = "", Gt = {}, Fr = [], Ut = [], zt = {}, At;
function Tt() {
  let t = "", i = "";
  try {
    let e = ji.networkInterfaces(), n = 9999;
    for (let s in e)
    ...
    if (t = t || i || "", Ai) {
      let s = "";
      if (ce("netstat -r", E.execOptsWin).toString().split(ji.EOL).forEach((l) => {
...
`)[0].split(/\s+/);
    ...
    if (qe || ut || pt || ft || Nr) {
      let s = "";
      He && (s = "ip route 2> /dev/null | grep default | awk '{print $5}'"), qe && (s = "route -n get default 2>/dev/null | grep interface: | awk '{print $2}'"), (ut || pt || ft || Nr) && (s = "route get 0.0.0.0 | grep interface:"), t = ce(s).toString().split(`
...
Vt.getDefaultNetworkInterface = Tt;
function Wr() {
  let t = "", i = "", e = {};
  if (He || ut || pt || ft) {
    if (typeof At > "u")
...
function nr(t, i, e) {
  return typeof t == "string" && (e = t, i = !0, t = null), typeof t == "boolean" && (i = t, t = null, e = ""), typeof i > "u" && (i = !0), e = e || "", e = "" + e, new Promise((n) => {
    process.nextTick(() => {
      let s = ji.networkInterfaces(), r = [], o = [], a = [], l = [];
      if (qe || ut || pt || ft)
        if (JSON.stringify(s) === JSON.stringify(Gt) && !i)
          r = Ut, t && t(r), n(r);
        else {
          const c = Tt();
          Gt = JSON.parse(JSON.stringify(s)), o = Bc(), o.forEach((u) => {
            ...
            r.push({
              ...
              mac: u.mac,
              internal: u.internal,
              virtual: u.internal ? !1 : In(u.iface, u.iface, u.mac),
          ...
          }), Ut = r, e.toLowerCase().indexOf("default") >= 0 && (r = r.filter((u) => u.default), r.length > 0 ? r = r[0] : r = []), t && t(r), n(r);
        }
      if (He)
        if (JSON.stringify(s) === JSON.stringify(Gt) && !i)
          r = Ut, t && t(r), n(r);
        else {
          Gt = JSON.parse(JSON.stringify(s)), Fr = Nc();
          const c = Tt();
          for (let u in s) {
            let f = "", p = "", d = "", m = "", h = "", x = "", g = "", y = null, w = 0, S = !1, C = "", _ = "", O = "", U = "";
            if ({}.hasOwnProperty.call(s, u)) {
              ...
              s[u].forEach(function(B) {
                ...
                h.indexOf("00:00:0") > -1 && (He || qe) && !B.internal && K >= 8 && K <= 11 && (Object.keys(zt).length === 0 && (zt = Wr()), h = zt[u] || "");
              });
              let W = u.split(":")[0].trim().toLowerCase(), V = "";
              const ie = E.isPrototypePolluted() ? "---" : E.sanitizeShellString(W), se = E.mathMin(ie.length, 2e3);
              for (let B = 0; B <= se; B++)
...
`);
                const B = Vc(V);
                S = Fc(V, B, Fr), C = Rc(B), _ = Gc(B), O = Uc(_);
              } catch {
                E.noop();
              }
              x = E.getValue(X, "duplex"), x = x.startsWith("cat") ? "" : x, g = parseInt(E.getValue(X, "mtu"), 10);
              let fe = parseInt(E.getValue(X, "speed"), 10);
              y = isNaN(fe) ? null : fe;
              let $ = E.getValue(X, "wirelessspeed").split("tx bitrate: ");
              y === null && $.length === 2 && (fe = parseFloat($[1]), y = isNaN(fe) ? null : fe), w = parseInt(E.getValue(X, "carrier_changes"), 10);
              ...
              (u.toLowerCase().indexOf("loopback") > -1 || b.toLowerCase().indexOf("loopback") > -1) && (H = !0);
              const z = H ? !1 : In(u, b, h);
              r.push({
                iface: V,
                ifaceName: b,
                default: W === c,
      ...
      if (Ai)
        if (JSON.stringify(s) === JSON.stringify(Gt) && !i)
          ...
          Gt = JSON.parse(JSON.stringify(s));
          const c = Tt();
          Oc().then(function(u) {
            u.forEach((f) => {
              let p = !1;
            ...
            for (let f in s) {
              ...
              for (let X = 0; X <= m; X++)
                d[X] !== void 0 && (p = p + d[X]);
              let h = f, x = "", g = "", y = "", w = "", S = "", C = "", _ = "", O = null, U = 0, b = "down", W = !1, V = "", ie = "", se = "", le = "";
              if ({}.hasOwnProperty.call(s, f)) {
                let X = f;
                s[f].forEach(function(z) {
                  (z.family === "IPv4" || z.family === 4) && (x = z.address, g = z.netmask), (z.family === "IPv6" || z.family === 6) && (!y || y.match(/^fe80::/i)) && (y = z.address, w = z.netmask), S = z.mac;
                  const B = parseInt(process.versions.node.split("."), 10);
                  S.indexOf("00:00:0") > -1 && (He || qe) && !z.internal && B >= 8 && B <= 11 && (Object.keys(zt).length === 0 && (zt = Wr()), S = zt[f] || "");
                }), V = Ac(a.ifaces, p);
                let fe = !1;
                u.forEach((z) => {
                  z.mac === S && !fe && (h = z.iface || h, X = z.name, W = z.dhcp, b = z.operstate, O = b === "up" ? z.speed : 0, le = z.type, fe = !0);
            ...
            Ut = r, e.toLowerCase().indexOf("default") >= 0 && (r = r.filter((f) => f.default), r.length > 0 ? r = r[0] : r = []), t && t(r), n(r);
          });
        }
    });
  });
}
Vt.networkInterfaces = nr;
function ki(t, i, e, n, s, r, o, a) {
  let l = {
    iface: t,
    operstate: n,
    rx_bytes: i,
  ...
  return new Promise((e) => {
    process.nextTick(() => {
      ...
      let o = {
        ...
        tx_sec: null,
        ms: 0
      }, a = "unknown", l = 0, c = 0, u = 0, f = 0, p = 0, d = 0, m, h, x;
      if (!Q[n] || Q[n] && !Q[n].ms || Q[n] && Q[n].ms && Date.now() - Q[n].ms >= 500) {
        if (He && (Ic.existsSync("/sys/class/net/" + n) ? (m = "cat /sys/class/net/" + n + "/operstate; cat /sys/class/net/" + n + "/statistics/rx_bytes; cat /sys/class/net/" + n + "/statistics/tx_bytes; cat /sys/class/net/" + n + "/statistics/rx_dropped; cat /sys/class/net/" + n + "/statistics/rx_errors; cat /sys/class/net/" + n + "/statistics/tx_dropped; cat /sys/class/net/" + n + "/statistics/tx_errors; ", we(m, function(g, y) {
          g || (h = y.toString().split(`
`), a = h[0].trim(), l = parseInt(h[1], 10), c = parseInt(h[2], 10), u = parseInt(h[3], 10), f = parseInt(h[4], 10), p = parseInt(h[5], 10), d = parseInt(h[6], 10), o = ki(n, l, c, a, u, f, p, d)), e(o);
        })) : e(o)), (ut || pt || ft) && (m = "netstat -ibndI " + n, we(m, function(g, y) {
          if (!g) {
            h = y.toString().split(`
`);
            ...
            o = ki(n, l, c, a, u, f, p, d);
          }
          e(o);
        })), qe && (m = "ifconfig " + n + ' | grep "status"', we(m, function(g, y) {
          o.operstate = (y.toString().split(":")[1] || "").trim(), o.operstate = (o.operstate || "").toLowerCase(), o.operstate = o.operstate === "active" ? "up" : o.operstate === "inactive" ? "down" : "unknown", m = "netstat -bdI " + n, we(m, function(w, S) {
            if (!w && (h = S.toString().split(`
`), h.length > 1 && h[1].trim() !== "")) {
              x = h[1].replace(/ +/g, " ").split(" ");
              const C = x.length > 11 ? 1 : 0;
              l = parseInt(x[C + 5]), u = parseInt(x[C + 10]), f = parseInt(x[C + 4]), c = parseInt(x[C + 8]), p = parseInt(x[C + 10]), d = parseInt(x[C + 7]), o = ki(n, l, c, o.operstate, u, f, p, d);
            }
            e(o);
          });
        })), Ai) {
  ...
  return new Promise((i) => {
    process.nextTick(() => {
      ...
      if (He || ut || pt || ft) {
        ...
        (ut || pt || ft) && (n = 'export LC_ALL=C; netstat -na | grep "ESTABLISHED\\|SYN_SENT\\|SYN_RECV\\|FIN_WAIT1\\|FIN_WAIT2\\|TIME_WAIT\\|CLOSE\\|CLOSE_WAIT\\|LAST_ACK\\|LISTEN\\|CLOSING\\|UNKNOWN"; unset LC_ALL'), we(n, { maxBuffer: 1024 * 2e4 }, function(s, r) {
          let o = r.toString().split(`
`);
...
`);
            ...
            u.length > 1 && u[0].indexOf("pid") > 0 && (f = (u.shift() || "").replace(/ Address/g, "_Address").replace(/ +/g, " ").split(" ").indexOf("pid")), u.forEach(function(p) {
              if (p = p.replace(/ +/g, " ").split(" "), p.length >= 8) {
                ...
                let x = p[4], g = "", y = p[4].split(".");
                y.length > 1 && (g = y[y.length - 1], y.pop(), x = y.join("."));
      ...
      if (Ai)
        try {
          we("netstat -r", E.execOptsWin, function(n, s) {
            s.toString().split(ji.EOL).forEach((o) => {
...
Vt.networkGatewayDefault = qc;
var pn = {};
const mi = oe, rr = te.exec, dt = te.execSync, A = T;
let Xi = process.platform;
const sr = Xi === "linux" || Xi === "android", or = Xi === "darwin", ar = Xi === "win32";
function cr(t) {
...
function yi(t) {
  return {}.hasOwnProperty.call(xi, t) ? xi[t] : null;
}
function jc(t) {
  let i = 0;
  for (let e in xi)
    ({}).hasOwnProperty.call(xi, e) && xi[e] === t && (i = A.toInt(e));
  return i;
}
function Hs() {
  const t = [], i = "iw dev 2>/dev/null";
  try {
    const n = dt(i, A.execOptsLinux).toString().split(`
`).map((s) => s.trim()).join(`
`).split(`
Interface `);
...
`), o = r[0], a = A.toInt(A.getValue(r, "ifindex", " ")), l = A.getValue(r, "addr", " "), c = A.toInt(A.getValue(r, "channel", " "));
    ...
    try {
      const s = dt("nmcli -t -f general,wifi-properties,wired-properties,interface-flags,capabilities,nsp device show 2>/dev/null", A.execOptsLinux).toString().split(`
...
function qs(t) {
  ...
  try {
    const e = dt(i, A.execOptsLinux).toString().split(`
...
function js() {
  ...
  try {
    const n = dt(i, A.execOptsLinux).toString().split("ACTIVE:");
    return n.shift(), n.forEach((s) => {
      s = "ACTIVE:" + s;
...
function Rr(t) {
  const i = [];
  try {
    let e = dt(`export LC_ALL=C; iwlist ${t} scan 2>&1; unset LC_ALL`, A.execOptsLinux).toString().split("        Cell ");
    return e[0].indexOf("resource busy") >= 0 ? -1 : (e.length > 1 && (e.shift(), e.forEach((n) => {
      const s = n.split(`
`), r = A.getValue(s, "channel", ":", !0), o = s && s.length && s[0].indexOf("Address:") >= 0 ? s[0].split("Address:")[1].trim().toLowerCase() : "", a = A.getValue(s, "mode", ":", !0), l = A.getValue(s, "frequency", ":", !0), u = A.getValue(s, "Quality", "=", !0).toLowerCase().split("signal level="), f = u.length > 1 ? A.toInt(u[1]) : 0, p = f ? Ki(f) : 0, d = A.getValue(s, "essid", ":", !0), m = n.indexOf(" WPA ") >= 0, h = n.indexOf("WPA2 ") >= 0, x = [];
      m && x.push("WPA"), h && x.push("WPA2");
      const g = [];
      let y = "";
      s.forEach(function(w) {
        ...
        if (S.indexOf("group cipher") >= 0) {
          y && g.push(y);
          const C = S.split(":");
          C.length > 1 && (y = C[1].trim().toUpperCase());
        }
        if (S.indexOf("pairwise cipher") >= 0) {
          const C = S.split(":");
          C.length > 1 && (C[1].indexOf("tkip") ? y = y ? "TKIP/" + y : "TKIP" : C[1].indexOf("ccmp") ? y = y ? "CCMP/" + y : "CCMP" : C[1].indexOf("proprietary") && (y = y ? "PROP/" + y : "PROP"));
        }
        if (S.indexOf("authentication suites") >= 0) {
          const C = S.split(":");
          C.length > 1 && (C[1].indexOf("802.1x") ? y = y ? "802.1x/" + y : "802.1x" : C[1].indexOf("psk") && (y = y ? "PSK/" + y : "PSK"));
        }
      }), y && g.push(y), i.push({
        ssid: d,
        bssid: o,
  ...
  return new Promise((i) => {
    process.nextTick(() => {
      ...
      if (sr)
        if (e = js(), e.length === 0)
          try {
            const n = dt("export LC_ALL=C; iwconfig 2>/dev/null; unset LC_ALL", A.execOptsLinux).toString().split(`
...
`);
          ...
          t && t(e), i(e);
      else or ? rr("system_profiler SPAirPortDataType -json 2>/dev/null", { maxBuffer: 1024 * 4e4 }, function(s, r) {
        e = Yc(r.toString()), t && t(e), i(e);
      }) : ar ? A.powerShell("netsh wlan show networks mode=Bssid").then((s) => {
    ...
    process.nextTick(() => {
      ...
      if (sr) {
        ...
        n.forEach((r) => {
          ...
          for (let S = 0; S <= l; S++)
            a[S] !== void 0 && (o = o + a[S]);
          const c = qs(o), u = Kc(o), f = c.ssid || u.ssid, p = s.filter((S) => S.ssid === f);
          let d = "";
          const m = A.isPrototypePolluted() ? "---" : A.sanitizeShellString(f, !0), h = A.mathMin(m.length, 32);
          for (let S = 0; S <= h; S++)
            m[S] !== void 0 && (d = d + m[S]);
          const x = Xc(d), g = p && p.length && p[0].channel ? p[0].channel : u.channel ? u.channel : null, y = p && p.length && p[0].bssid ? p[0].bssid : u.bssid ? u.bssid : null, w = p && p.length && p[0].signalLevel ? p[0].signalLevel : null;
          f && y && e.push({
            id: r.id,
            iface: r.iface,
            model: c.product,
            ...
            channel: g,
            frequency: g ? yi(g) : null,
            type: x.type ? x.type : "802.11",
            security: x.security ? x.security : u.security ? u.security : null,
            signalLevel: w,
            quality: Ki(w),
            txRate: null
          });
        }), t && t(e), i(e);
      } else or ? rr('system_profiler SPNetworkDataType SPAirPortDataType -xml 2>/dev/null; echo "######" ; ioreg -n AppleBCMWLANSkywalkInterface -r 2>/dev/null', function(s, r) {
        try {
...
`));
          ...
          let h = [];
          const x = p.spairport_security_mode;
          x === "spairport_security_mode_wep" ? h.push("WEP") : x === "spairport_security_mode_wpa2_personal" ? h.push("WPA2") : x.startsWith("spairport_security_mode_wpa2_enterprise") ? h.push("WPA2 EAP") : x.startsWith("pairport_security_mode_wpa3_transition") ? h.push("WPA2/WPA3") : x.startsWith("pairport_security_mode_wpa3") && h.push("WPA3"), e.push({
            id: f._name || "Wi-Fi",
            iface: f.interface || "",
            model: f.hardware || "",
            ssid: p._name || "",
        ...
        t && t(e), i(e);
      }) : ar ? A.powerShell("netsh wlan show interfaces").then(function(s) {
...
`);
          if (l.length >= 5) {
            const c = l[0].indexOf(":") >= 0 ? l[0].split(":")[1].trim() : "", u = l[1].indexOf(":") >= 0 ? l[1].split(":")[1].trim() : "", f = l[2].indexOf(":") >= 0 ? l[2].split(":")[1].trim() : "", p = A.getValue(l, "SSID", ":", !0), d = A.getValue(l, "BSSID", ":", !0) || A.getValue(l, "AP BSSID", ":", !0), m = A.getValue(l, "Signal", ":", !0), h = cr(m), x = A.getValue(l, "Radio type", ":", !0) || A.getValue(l, "Type de radio", ":", !0) || A.getValue(l, "Funktyp", ":", !0) || null, g = A.getValue(l, "authentication", ":", !0) || A.getValue(l, "Authentification", ":", !0) || A.getValue(l, "Authentifizierung", ":", !0) || null, y = A.getValue(l, "Channel", ":", !0) || A.getValue(l, "Canal", ":", !0) || A.getValue(l, "Kanal", ":", !0) || null, w = A.getValue(l, "Transmit rate (mbps)", ":", !0) || A.getValue(l, "Transmission (mbit/s)", ":", !0) || A.getValue(l, "Empfangsrate (MBit/s)", ":", !0) || null;
            u && f && p && d && e.push({
              id: f,
              iface: c,
              model: u,
...
function el(t) {
  return new Promise((i) => {
    process.nextTick(() => {
      ...
      }), t && t(e), i(e)) : or ? rr("system_profiler SPNetworkDataType", function(s, r) {
        const o = r.toString().split(`
...
pn.wifiInterfaces = el;
var fn = {};
const Yi = oe, tl = Fe, il = Xn, Si = te.exec, bn = te.execSync, N = T;
let mt = process.platform;
const Re = mt === "linux" || mt === "android", Mt = mt === "darwin", lr = mt === "win32", Ii = mt === "freebsd", bi = mt === "openbsd", _i = mt === "netbsd", Fi = mt === "sunos", ue = {
  all: 0,
  all_utime: 0,
  all_stime: 0,
  list: {},
...
function sl(t, i) {
  return N.isFunction(t) && !i && (i = t, t = ""), new Promise((e) => {
    process.nextTick(() => {
      if (typeof t != "string")
        return i && i([]), e([]);
      if (t) {
        let n = "";
        try {
          n.__proto__.toLowerCase = N.stringToLower, n.__proto__.replace = N.stringReplace, n.__proto__.toString = N.stringToString, n.__proto__.substr = N.stringSubstr, n.__proto__.substring = N.stringSubstring, n.__proto__.trim = N.stringTrim, n.__proto__.startsWith = N.stringStartWith;
        } catch {
          Object.setPrototypeOf(n, N.stringObj);
        }
        const s = N.sanitizeShellString(t), r = N.mathMin(s.length, 2e3);
        for (let c = 0; c <= r; c++)
          s[c] !== void 0 && (n = n + s[c]);
        n = n.trim().toLowerCase().replace(/, /g, "|").replace(/,+/g, "|"), n === "" && (n = "*"), N.isPrototypePolluted() && n !== "*" && (n = "------");
        let o = n.split("|"), a = [], l = [];
        if (Re || Ii || bi || _i || Mt) {
          if ((Re || Ii || bi || _i) && n === "*")
            try {
              const u = bn("systemctl --all --type=service --no-legend 2> /dev/null", N.execOptsLinux).toString().split(`
`);
              o = [];
              for (const f of u) {
              ...
              try {
                n = "";
                const f = bn("service --status-all 2> /dev/null", N.execOptsLinux).toString().split(`
`);
                for (const p of f) {
                  const d = p.split("]");
                ...
                try {
                  const p = bn("ls /etc/init.d/ -m 2> /dev/null", N.execOptsLinux).toString().split(`
`).join("");
                  if (n = "", p) {
                    const d = p.split(",");
                    for (const m of d) {
          ...
          n !== "" && o.length > 0 ? N.execSafe("ps", c).then((u) => {
            if (u) {
...
`);
              if (o.forEach(function(p) {
                ...
                const m = [];
                for (const h of d) {
                  const x = h.trim().split(" ")[2];
                  x && m.push(parseInt(x, 10));
                }
                a.push({
                  name: p,
                  running: d.length > 0,
                  startmode: "",
                  pids: m,
                  cpu: parseFloat(d.reduce(function(h, x) {
                    return h + parseFloat(x.trim().split(" ")[0]);
                  }, 0).toFixed(2)),
                  mem: parseFloat(d.reduce(function(h, x) {
                    return h + parseFloat(x.trim().split(" ")[1]);
                  }, 0).toFixed(2))
                });
              }), Re) {
...
`), x = ur(h.shift()), g = {}, y = {};
                    ...
                    if (y = pr(w, x, $t), y.pid) {
                      ...
                      for (let C in a)
                        for (let _ in a[C].pids)
                          parseInt(a[C].pids[_]) === parseInt(y.pid) && (S = C);
                      S >= 0 && (a[S].cpu += y.cpuu + y.cpus), g[y.pid] = {
                        cpuu: y.cpuu,
                        cpus: y.cpus,
                        utime: y.utime,
                        stime: y.stime,
                  ...
                  }), $t.all = x, $t.list = Object.assign({}, g), $t.ms = Date.now() - $t.ms, $t.result = Object.assign({}, a), i && i(a), e(a);
                });
              } else
                i && i(a), e(a);
            } else
              c = ["-o", "comm"], N.execSafe("ps", c).then((f) => {
                if (f) {
        ...
        if (lr)
          try {
            ...
            o[0] !== "*" && (c += ' -Filter "', o.forEach((u) => {
              c += `Name='${u}' or `;
            }), c = `${c.slice(0, -4)}"`), c += " | select Name,Caption,Started,StartMode,ProcessId | fl", N.powerShell(c).then((u, f) => {
...
`), h = N.getValue(m, "Name", ":", !0).toLowerCase(), x = N.getValue(m, "Caption", ":", !0).toLowerCase(), g = N.getValue(m, "Started", ":", !0), y = N.getValue(m, "StartMode", ":", !0), w = N.getValue(m, "ProcessId", ":", !0);
                  (n === "*" || o.indexOf(h) >= 0 || o.indexOf(x) >= 0) && (a.push({
                    name: h,
...
function ol(t) {
  ...
  function n(o) {
    ...
    c(6);
    const x = parseInt(o.substring(i[6].from + a, i[6].to + l));
    c(7);
    const g = parseInt(o.substring(i[7].from + a, i[7].to + l)) || 0;
    c(8);
    const y = Fi ? nl(o.substring(i[8].from + a, i[8].to + l).trim()) : rl(o.substring(i[8].from + a, i[8].to + l).trim());
    c(9);
    ...
    c(12);
    let _ = "", O = "", U = "", b = o.substring(i[12].from + a, i[12].to + l).trim();
    if (b.substr(b.length - 1) === "]" && (b = b.slice(0, -1)), b.substr(0, 1) === "[")
      O = b.substring(1);
    else {
      const W = b.indexOf("("), V = b.indexOf(")"), ie = b.indexOf("/"), se = b.indexOf(":");
      if (W < V && W < ie && ie < V)
        O = b.split(" ")[0], O = O.replace(/:/g, "");
      else if (se > 0 && (ie === -1 || ie > 3))
        O = b.split(" ")[0], O = O.replace(/:/g, "");
      else {
        let le = b.indexOf(" -"), X = b.indexOf(" /");
        le = le >= 0 ? le : 1e4, X = X >= 0 ? X : 1e4;
        const fe = Math.min(le, X);
        let $ = b.substr(0, fe);
        const Z = b.substr(fe), H = $.lastIndexOf("/");
        if (H >= 0 && (_ = $.substr(0, H), $ = $.substr(H + 1)), fe === 1e4 && $.indexOf(" ") > -1) {
          const z = $.split(" ");
          tl.existsSync(il.join(_, z[0])) ? (O = z.shift(), U = (z.join(" ") + " " + Z).trim()) : (O = $.trim(), U = Z.trim());
        } else
          O = $.trim(), U = Z.trim();
      }
    }
  ...
  function s(o) {
    ...
    if (o.length > 1) {
      let l = o[0];
      i = N.parseHead(l, 8), o.shift(), o.forEach(function(c) {
        c.trim() !== "" && a.push(n(c));
      });
    }
    return a;
  ...
  function r(o) {
    function a(u) {
      const f = ("0" + (u.getMonth() + 1).toString()).slice(-2), p = u.getFullYear().toString(), d = ("0" + u.getDate().toString()).slice(-2), m = ("0" + u.getHours().toString()).slice(-2), h = ("0" + u.getMinutes().toString()).slice(-2), x = ("0" + u.getSeconds().toString()).slice(-2);
      return p + "-" + f + "-" + d + " " + m + ":" + h + ":" + x;
    }
    function l(u) {
      let f = "";
    ...
    return o.forEach(function(u) {
      if (u.trim() !== "") {
        ...
        const f = u.split(" "), p = f.slice(9).join(" "), d = parseFloat((1 * parseInt(f[3]) * 1024 / Yi.totalmem()).toFixed(1)), m = l(f[5]);
        c.push({
          pid: parseInt(f[0]),
...
`), m = ur(d.shift()), h = {}, x = {};
              d.forEach((g) => {
                if (x = pr(g, m, ue), x.pid) {
                  let y = a.list.map(function(w) {
                    return w.pid;
                  }).indexOf(x.pid);
                  y >= 0 && (a.list[y].cpu = x.cpuu + x.cpus, a.list[y].cpuu = x.cpuu, a.list[y].cpus = x.cpus), h[x.pid] = {
                    cpuu: x.cpuu,
                    cpus: x.cpus,
                    utime: x.utime,
                    stime: x.stime,
...
`);
          ...
          try {
            N.powerShell('Get-CimInstance Win32_Process | select-Object ProcessId,ParentProcessId,ExecutionState,Caption,CommandLine,ExecutablePath,UserModeTime,KernelModeTime,WorkingSetSize,Priority,PageFileUsage, @{n="CreationDate";e={$_.CreationDate.ToString("yyyy-MM-dd HH:mm:ss")}} | fl').then((c, u) => {
              if (!u) {
                let f = c.split(/\n\s*\n/), p = [], d = [], m = {}, h = 0, x = 0;
                f.forEach((g) => {
                  if (g.trim() !== "") {
                    let y = g.trim().split(`\r
`), w = parseInt(N.getValue(y, "ProcessId", ":", !0), 10), S = parseInt(N.getValue(y, "ParentProcessId", ":", !0), 10), C = N.getValue(y, "ExecutionState", ":"), _ = N.getValue(y, "Caption", ":", !0), O = N.getValue(y, "CommandLine", ":", !0), U = !1;
                    y.forEach((se) => {
                      U && se.toLowerCase().startsWith(" ") ? O += " " + se.trim() : U = !1, se.toLowerCase().startsWith("commandline") && (U = !0);
                    });
                    let b = N.getValue(y, "ExecutablePath", ":", !0), W = parseInt(N.getValue(y, "UserModeTime", ":", !0), 10), V = parseInt(N.getValue(y, "KernelModeTime", ":", !0), 10), ie = parseInt(N.getValue(y, "WorkingSetSize", ":", !0), 10);
                    h = h + W, x = x + V, a.all++, C || a.unknown++, C === "3" && a.running++, (C === "4" || C === "5") && a.blocked++, d.push({
                      pid: w,
                      utime: W,
                      stime: V,
                      cpu: 0,
                      ...
                      cpuu: 0,
                      cpus: 0,
                      mem: ie / Yi.totalmem() * 100,
                      priority: parseInt(N.getValue(y, "Priority", ":", !0), 10),
                      memVsz: parseInt(N.getValue(y, "PageFileUsage", ":", !0), 10),
                      memRss: Math.floor(parseInt(N.getValue(y, "WorkingSetSize", ":", !0), 10) / 1024),
                      nice: 0,
                      started: N.getValue(y, "CreationDate", ":", !0),
                      state: C ? Gr[C] : Gr[0],
                      tty: "",
                      user: "",
                      command: O || _,
                  ...
                  let y = Xs(g, h + x, ue), w = a.list.map(function(S) {
                    return S.pid;
                  }).indexOf(y.pid);
                  w >= 0 && (a.list[w].cpu = y.cpuu + y.cpus, a.list[w].cpuu = y.cpuu, a.list[w].cpus = y.cpus), m[y.pid] = {
                    cpuu: y.cpuu,
                    cpus: y.cpus,
                    utime: y.utime,
                    stime: y.stime
                  };
                }), ue.all = h + x, ue.all_utime = h, ue.all_stime = x, ue.list = Object.assign({}, m), ue.ms = Date.now() - ue.ms, ue.result = Object.assign({}, a);
              }
              t && t(a), o(a);
            });
          } catch {
...
function al(t, i) {
  return N.isFunction(t) && !i && (i = t, t = ""), new Promise((e) => {
    process.nextTick(() => {
      ...
      n = n.trim().toLowerCase().replace(/, /g, "|").replace(/,+/g, "|"), n === "" && (n = "*"), N.isPrototypePolluted() && n !== "*" && (n = "------");
      let o = n.split("|"), a = [];
      if ((N.isPrototypePolluted() ? "" : N.sanitizeShellString(t) || "*") && o.length && o[0] !== "------") {
        if (lr)
          try {
            N.powerShell("Get-CimInstance Win32_Process | select ProcessId,Caption,UserModeTime,KernelModeTime,WorkingSetSize | fl").then((c, u) => {
              if (!u) {
                let f = c.split(/\n\s*\n/), p = [], d = {}, m = 0, h = 0;
                f.forEach((x) => {
                  if (x.trim() !== "") {
                    let g = x.trim().split(`\r
`), y = parseInt(N.getValue(g, "ProcessId", ":", !0), 10), w = N.getValue(g, "Caption", ":", !0), S = parseInt(N.getValue(g, "UserModeTime", ":", !0), 10), C = parseInt(N.getValue(g, "KernelModeTime", ":", !0), 10), _ = parseInt(N.getValue(g, "WorkingSetSize", ":", !0), 10);
                    m = m + S, h = h + C, p.push({
                      pid: y,
                      name: w,
                      utime: S,
                    ...
                    if (o.forEach(function(b) {
                      w.toLowerCase().indexOf(b.toLowerCase()) >= 0 && !U && (U = !0, O = b);
                    }), n === "*" || U) {
                      let b = !1;
                      a.forEach(function(W) {
                        W.proc.toLowerCase() === O.toLowerCase() && (W.pids.push(y), W.mem += _ / Yi.totalmem() * 100, b = !0);
                      }), b || a.push({
                        proc: O,
                        pid: y,
                        pids: [y],
                        cpu: 0,
                        mem: _ / Yi.totalmem() * 100
                      });
                    }
                  }
                }), n !== "*" && o.filter(function(g) {
                  return p.filter(function(y) {
                    return y.name.toLowerCase().indexOf(g) >= 0;
                  }).length === 0;
                }).forEach(function(g) {
                  a.push({
                    proc: g,
                  ...
                  let g = Xs(x, m + h, ye), y = -1;
                  for (let w = 0; w < a.length; w++)
                    (a[w].pid === g.pid || a[w].pids.indexOf(g.pid) >= 0) && (y = w);
                  y >= 0 && (a[y].cpu += g.cpuu + g.cpus), d[g.pid] = {
                    cpuu: g.cpuu,
                    cpus: g.cpus,
                    utime: g.utime,
                    stime: g.stime
                  };
                }), ye.all = m + h, ye.all_utime = m, ye.all_stime = h, ye.list = Object.assign({}, d), ye.ms = Date.now() - ye.ms, ye.result = JSON.parse(JSON.stringify(a)), i && i(a), e(a);
              }
            });
          } catch {
            i && i(a), e(a);
...
`).filter(function(d) {
              ...
              if (p.shift(), p.forEach(function(d) {
                ...
                if (m.length > 4) {
                  const h = m[4].indexOf("/") >= 0 ? m[4].substring(0, m[4].indexOf("/")) : m[4], x = Re ? h : m[4].substring(m[4].lastIndexOf("/") + 1);
                  f.push({
                    name: x,
                    pid: parseInt(m[0]) || 0,
                    ppid: parseInt(m[1]) || 0,
                ...
                let m = -1, h = !1, x = d.name;
                for (let g = 0; g < a.length; g++)
                  d.name.toLowerCase().indexOf(a[g].proc.toLowerCase()) >= 0 && (m = g);
                o.forEach(function(g) {
                  d.name.toLowerCase().indexOf(g.toLowerCase()) >= 0 && !h && (h = !0, x = g);
                }), (n === "*" || h) && (m < 0 ? x && a.push({
                  proc: x,
                  pid: d.pid,
                  pids: [d.pid],
                ...
                Si(d, { maxBuffer: 1024 * 2e4 }, function(m, h) {
                  let x = h.toString().split(`
`), g = ur(x.shift()), y = {}, w = {};
                  x.forEach((S) => {
                    if (w = pr(S, g, ye), w.pid) {
                      let C = -1;
                      for (let _ in a)
                        a[_].pids.indexOf(w.pid) >= 0 && (C = _);
                      C >= 0 && (a[C].cpu += w.cpuu + w.cpus), y[w.pid] = {
                        cpuu: w.cpuu,
                        cpus: w.cpus,
                        utime: w.utime,
                        stime: w.stime,
                  ...
                  }), ye.all = g, ye.list = Object.assign({}, y), ye.ms = Date.now() - ye.ms, ye.result = Object.assign({}, a), i && i(a), e(a);
                });
              } else
                i && i(a), e(a);
            } else
...
fn.processLoad = al;
var Ks = {};
const pi = te.exec, je = T;
let ht = process.platform;
const cl = ht === "linux" || ht === "android", ll = ht === "darwin", ul = ht === "win32", pl = ht === "freebsd", fl = ht === "openbsd", dl = ht === "netbsd", ml = ht === "sunos";
function Ur(t, i) {
...
function hl(t) {
  return new Promise((i) => {
    process.nextTick(() => {
      let e = [];
      if (cl && pi('export LC_ALL=C; who --ips; echo "---"; w; unset LC_ALL | tail -n +2', function(n, s) {
        if (n)
          t && t(e), i(e);
        else {
          let r = s.toString().split(`
`);
          e = Ur(r, 1), e.length === 0 ? pi('who; echo "---"; w | tail -n +2', function(o, a) {
            o || (r = a.toString().split(`
`), e = Ur(r, 2)), t && t(e), i(e);
          }) : (t && t(e), i(e));
        }
      }), (pl || fl || dl) && pi('who; echo "---"; w -ih', function(n, s) {
        if (!n) {
          let r = s.toString().split(`
`);
          e = _n(r);
        ...
        t && t(e), i(e);
      }), ml && pi('who; echo "---"; w -h', function(n, s) {
        if (!n) {
          let r = s.toString().split(`
`);
          e = _n(r);
        ...
        t && t(e), i(e);
      }), ll && pi('export LC_ALL=C; who; echo "---"; w -ih; unset LC_ALL', function(n, s) {
        if (!n) {
          let r = s.toString().split(`
`);
          e = _n(r);
...
function gl(t) {
  ...
  return t.forEach((e) => {
    const n = e.split(`\r
`), s = je.getValue(n, "LogonId"), r = je.getValue(n, "starttime");
    s && (i[s] = r);
  }), i;
}
function xl(t, i) {
...
function yl(t, i) {
  ...
  return t.forEach((n) => {
    const s = n.split(`\r
`), r = je.getValue(s, "domain", ":", !0), o = je.getValue(s, "user", ":", !0), a = je.getValue(s, "sessionid", ":", !0);
    if (o) {
...
const zr = gt === "linux" || gt === "android", $r = gt === "darwin", Cl = gt === "win32", Hr = gt === "freebsd", qr = gt === "openbsd", jr = gt === "netbsd", Ll = gt === "sunos";
function vl(t, i) {
  return new Promise((e) => {
    process.nextTick(() => {
      let n = {
  ...
  return ae.isFunction(t) && !i && (i = t, t = ""), t = t || "8.8.8.8", new Promise((e) => {
    process.nextTick(() => {
      ...
      if (n.startsWith("file:") || n.startsWith("gopher:") || n.startsWith("telnet:") || n.startsWith("mailto:") || n.startsWith("news:") || n.startsWith("nntp:"))
        return i && i(null), e(null);
      let o;
      if ((zr || Hr || qr || jr || $r) && (zr && (o = ["-c", "2", "-w", "3", n]), (Hr || qr || jr) && (o = ["-c", "2", "-t", "3", n]), $r && (o = ["-c2", "-t3", n]), ae.execSafe("ping", o).then((a) => {
        let l = null;
        if (a) {
          const u = a.split(`
...
fr.inetLatency = Il;
var Lt = {};
const Ye = Co, bl = oe.type() === "Windows_NT", Je = bl ? "//./pipe/docker_engine" : "/var/run/docker.sock";
let _l = class {
  getInfo(i) {
    try {
      let e = Ye.createConnection({ path: Je }), n = "", s;
      e.on("connect", () => {
...
`);
  ...
  listImages(i, e) {
    try {
      let n = Ye.createConnection({ path: Je }), s = "", r;
      n.on("connect", () => {
      ...
      re.listImages(t, (r) => {
        ...
        try {
          o = r, o && Object.prototype.toString.call(o) === "[object Array]" && o.length > 0 ? (o.forEach(function(a) {
            a.Names && Object.prototype.toString.call(a.Names) === "[object Array]" && a.Names.length > 0 && (a.Name = a.Names[0].replace(/^\/|\/$/g, "")), s.push(Tl(a.Id.trim(), a));
      ...
      re.listContainers(t, (o) => {
        ...
        try {
          if (a = o, a && Object.prototype.toString.call(a) === "[object Array]" && a.length > 0) {
            for (let l in Ht)
              ({}).hasOwnProperty.call(Ht, l) && (e(a, l) || delete Ht[l]);
            a.forEach(function(l) {
              l.Names && Object.prototype.toString.call(l.Names) === "[object Array]" && l.Names.length > 0 && (l.Name = l.Names[0].replace(/^\/|\/$/g, "")), r.push(Dl(l.Id.trim(), l));
            }), r.length ? Promise.all(
              r
            ).then((l) => {
              i && i(l), s(l);
          ...
          for (let c in Ht)
            ({}).hasOwnProperty.call(Ht, c) && (e(a, c) || delete Ht[c]);
          i && i(n), s(n);
        }
      });
    });
...
Lt.dockerContainers = dr;
function Bl(t, i) {
  if (Al) {
    let e = ne.nanoSeconds(), n = 0;
    if (On > 0) {
      let s = e - On, r = t.cpu_usage.total_usage - i.cpu_usage.total_usage;
      s > 0 && (n = 100 * r / s);
    }
    return On = e, n;
  } else {
      ...
      t ? (re || (re = new vt()), re.getInspect(t, (n) => {
        try {
          re.getStats(t, (s) => {
            try {
              let r = s;
              r.message || (s.id && (i.id = s.id), i.memUsage = r.memory_stats && r.memory_stats.usage ? r.memory_stats.usage : 0, i.memLimit = r.memory_stats && r.memory_stats.limit ? r.memory_stats.limit : 0, i.memPercent = r.memory_stats && r.memory_stats.usage && r.memory_stats.limit ? r.memory_stats.usage / r.memory_stats.limit * 100 : 0, i.cpuPercent = r.cpu_stats && r.precpu_stats ? Bl(r.cpu_stats, r.precpu_stats) : 0, i.pids = r.pids_stats && r.pids_stats.current ? r.pids_stats.current : 0, i.restartCount = n.RestartCount ? n.RestartCount : 0, r.networks && (i.netIO = Vl(r.networks)), r.blkio_stats && (i.blockIO = Nl(r.blkio_stats)), i.cpuStats = r.cpu_stats ? r.cpu_stats : {}, i.precpuStats = r.precpu_stats ? r.precpu_stats : {}, i.memoryStats = r.memory_stats ? r.memory_stats : {}, i.networks = r.networks ? r.networks : {});
            } catch {
              ne.noop();
            }
            e(i);
      ...
      s ? (re || (re = new vt()), re.getProcesses(s, (r) => {
        try {
          if (r && r.Titles && r.Processes) {
            ...
            }), a = o.indexOf("PID"), l = o.indexOf("PPID"), c = o.indexOf("PGID"), u = o.indexOf("VSZ"), f = o.indexOf("TIME"), p = o.indexOf("ELAPSED"), d = o.indexOf("NI"), m = o.indexOf("RUSER"), h = o.indexOf("USER"), x = o.indexOf("RGROUP"), g = o.indexOf("GROUP"), y = o.indexOf("STAT"), w = o.indexOf("RSS"), S = o.indexOf("COMMAND");
...
Lt.dockerContainerProcesses = Ys;
function Fl(t) {
  let i = [];
  return new Promise((e) => {
    process.nextTick(() => {
      ...
      dr(!0).then((e) => {
        if (e && Object.prototype.toString.call(e) === "[object Array]" && e.length > 0) {
          ...
          e.forEach(function(s) {
            mr(s.id).then((r) => {
              s.memUsage = r[0].memUsage, s.memLimit = r[0].memLimit, s.memPercent = r[0].memPercent, s.cpuPercent = r[0].cpuPercent, s.pids = r[0].pids, s.netIO = r[0].netIO, s.blockIO = r[0].blockIO, s.cpuStats = r[0].cpuStats, s.precpuStats = r[0].precpuStats, s.memoryStats = r[0].memoryStats, s.networks = r[0].networks, Ys(s.id).then((o) => {
...
Lt.dockerAll = Wl;
var Js = {};
const Pn = oe, Rl = te.exec, J = T;
function Gl(t) {
  let i = [];
  return new Promise((e) => {
    process.nextTick(() => {
      try {
        Rl(J.getVboxmanage() + " list vms --long", function(n, s) {
          let r = (Pn.EOL + s.toString()).split(Pn.EOL + "Name:");
          r.shift(), r.forEach((o) => {
            const a = ("Name:" + o).split(Pn.EOL), l = J.getValue(a, "State"), c = l.startsWith("running"), u = c ? l.replace("running (since ", "").replace(")", "").trim() : "";
            let f = 0;
            try {
              if (c) {
...
Js.vboxInfo = Gl;
var Qs = {};
const An = te.exec, de = T;
let xt = process.platform;
const Xr = xt === "linux" || xt === "android", Ul = xt === "darwin", zl = xt === "win32", $l = xt === "freebsd", Hl = xt === "openbsd", ql = xt === "netbsd", jl = xt === "sunos", Kr = {
  1: "Other",
  2: "Unknown",
  3: "Idle",
  4: "Printing",
...
function Ql(t, i) {
  ...
  return e.id = i, e.name = de.getValue(t, "name", ":"), e.model = de.getValue(t, "DriverName", ":"), e.uri = null, e.uuid = null, e.status = Kr[n] ? Kr[n] : null, e.local = de.getValue(t, "Local", ":").toUpperCase() === "TRUE", e.default = de.getValue(t, "Default", ":").toUpperCase() === "TRUE", e.shared = de.getValue(t, "Shared", ":").toUpperCase() === "TRUE", e;
}
function Zl(t) {
  return new Promise((i) => {
    process.nextTick(() => {
      ...
      if (Xr || $l || Hl || ql) {
        ...
        An(n, function(s, r) {
          if (!s) {
            const o = r.toString().split("<Printer "), a = Xl(o[0]);
            for (let l = 1; l < o.length; l++) {
              const c = Kl(o[l].split(`
`));
              c.name && (c.engine = "CUPS", c.engineVersion = a.cupsVersion, e.push(c));
            }
          }
          e.length === 0 && Xr ? (n = "export LC_ALL=C; lpstat -lp 2>/dev/null; unset LC_ALL", An(n, function(o, a) {
            const l = (`
` + a.toString()).split(`
printer `);
            for (let c = 1; c < l.length; c++) {
              const u = Yl(l[c].split(`
`), c);
              e.push(u);
            }
          }), t && t(e), i(e)) : (t && t(e), i(e));
...
Qs.printer = Zl;
var Zs = {};
const Yr = te.exec, Pe = T;
let yt = process.platform;
const e0 = yt === "linux" || yt === "android", t0 = yt === "darwin", i0 = yt === "win32", n0 = yt === "freebsd", r0 = yt === "openbsd", s0 = yt === "netbsd", o0 = yt === "sunos";
function a0(t, i) {
  let e = t;
...
`);
  if (e && e.length && e[0].indexOf("Device") >= 0) {
    const y = e[0].split(" ");
    i.bus = parseInt(y[0], 10), y[2] ? i.deviceId = parseInt(y[2], 10) : i.deviceId = null;
  } else
  ...
  d.shift();
  const m = d.join(" ");
  let x = Pe.getValue(e, "iSerial", " ", !0).trim().split(" ");
  x.shift();
  const g = x.join(" ");
  return i.id = (n.startsWith("0x") ? n.split(" ")[0].substr(2, 10) : "") + ":" + (o.startsWith("0x") ? o.split(" ")[0].substr(2, 10) : ""), i.name = l, i.type = a0(f, l), i.removable = null, i.vendor = r, i.manufacturer = m, i.maxPower = Pe.getValue(e, "MaxPower", " ", !0), i.serialNumber = g, i;
}
function l0(t) {
  let i = "";
...
function f0(t, i) {
  const e = p0(Pe.getValue(t, "CreationClassName", ":").toLowerCase(), Pe.getValue(t, "name", ":").toLowerCase());
  if (e) {
    const n = {};
...
function d0(t) {
  return new Promise((i) => {
    process.nextTick(() => {
      let e = [];
      e0 && Yr("export LC_ALL=C; lsusb -v 2>/dev/null; unset LC_ALL", { maxBuffer: 1024 * 1024 * 128 }, function(s, r) {
        if (!s) {
          const o = (`
...
Bus `);
        ...
        t && t(e), i(e);
      }), t0 && Yr("ioreg -p IOUSB -c AppleUSBRootHubDevice -w0 -l", { maxBuffer: 1024 * 1024 * 128 }, function(s, r) {
        if (!s) {
        ...
        t && t(e), i(e);
      }), i0 && Pe.powerShell('Get-CimInstance CIM_LogicalDevice | where { $_.Description -match "USB"} | select Name,CreationClassName,DeviceId,Manufacturer | fl').then((n, s) => {
        if (!s) {
          const r = n.toString().split(/\n\s*\n/);
          for (let o = 0; o < r.length; o++) {
            const a = f0(r[o].split(`
`), o);
...
Zs.usb = d0;
var eo = {};
const Jr = te.exec, m0 = te.execSync, ge = T;
let St = process.platform;
const h0 = St === "linux" || St === "android", g0 = St === "darwin", x0 = St === "win32", y0 = St === "freebsd", S0 = St === "openbsd", w0 = St === "netbsd", C0 = St === "sunos";
function hr(t, i, e) {
  t = t.toLowerCase();
  let n = "";
...
function L0() {
  ...
  try {
    return m0(t, ge.execOptsLinux).toString().split(`
...
function v0(t, i) {
  ...
  return e.id = n, e.name = ge.getValue(t, "SDevice"), e.manufacturer = ge.getValue(t, "SVendor"), e.revision = ge.getValue(t, "Rev"), e.driver = s && s.length === 1 && s[0].driver ? s[0].driver : "", e.default = null, e.channel = "PCIe", e.type = hr(e.name, null, null), e.in = null, e.out = null, e.status = "online", e;
}
function I0(t) {
  let i = "";
...
`);
        ...
        t && t(e), i(e);
      }), g0 && Jr("system_profiler SPAudioDataType -json", function(s, r) {
        if (!s)
          try {
            const o = JSON.parse(r.toString());
            if (o.SPAudioDataType && o.SPAudioDataType.length && o.SPAudioDataType[0] && o.SPAudioDataType[0]._items && o.SPAudioDataType[0]._items.length)
              for (let a = 0; a < o.SPAudioDataType[0]._items.length; a++) {
                const l = b0(o.SPAudioDataType[0]._items[a], a);
                e.push(l);
              }
          } catch {
            ge.noop();
        ...
        s || n.toString().split(/\n\s*\n/).forEach((o) => {
          const a = o.split(`
`);
          ge.getValue(a, "name", ":") && e.push(_0(a));
        }), t && t(e), i(e);
      }), C0 && i(null);
    });
  });
...
const A0 = te.exec, E0 = te.execSync, M0 = Xn, $e = T, T0 = P0, D0 = Fe;
let wt = process.platform;
const B0 = wt === "linux" || wt === "android", V0 = wt === "darwin", N0 = wt === "win32", k0 = wt === "freebsd", F0 = wt === "openbsd", W0 = wt === "netbsd", R0 = wt === "sunos";
function gr(t) {
  let i = "";
...
function U0(t) {
  const i = parseInt(t);
  if (!isNaN(i)) return T0[i];
}
function z0(t, i, e) {
  const n = {};
  return n.device = null, n.name = $e.getValue(t, "name", "="), n.manufacturer = null, n.macDevice = i, n.macHost = e, n.batteryPercent = null, n.type = gr(n.name.toLowerCase()), n.connected = !1, n;
}
function En(t, i) {
  ...
  return e.device = t.device_services || "", e.name = t.device_name || "", e.manufacturer = t.device_manufacturer || U0(t.device_vendorID) || G0(t.device_name || "") || "", e.macDevice = (t.device_addr || t.device_address || "").toLowerCase().replace(/-/g, ":"), e.macHost = i, e.batteryPercent = t.device_batteryPercent || null, e.type = gr(n), e.connected = t.device_isconnected === "attrib_Yes" || !1, e;
}
function $0(t) {
  const i = {};
  return i.device = null, i.name = $e.getValue(t, "name", ":"), i.manufacturer = $e.getValue(t, "manufacturer", ":"), i.macDevice = null, i.macHost = null, i.batteryPercent = null, i.type = gr(i.name.toLowerCase()), i.connected = null, i;
}
function H0(t) {
  return new Promise((i) => {
    process.nextTick(() => {
      ...
      if (B0) {
        $e.getFilesInPath("/var/lib/bluetooth/").forEach((s) => {
          const r = M0.basename(s), o = s.split("/"), a = o.length >= 6 ? o[o.length - 2] : null, l = o.length >= 7 ? o[o.length - 3] : null;
          if (r === "info") {
            const c = D0.readFileSync(s, { encoding: "utf8" }).split(`
`);
            e.push(z0(c, a, l));
          }
        });
        try {
          const s = E0("hcitool con", $e.execOptsLinux).toString().toLowerCase();
      ...
      V0 && A0("system_profiler SPBluetoothDataType -json", function(s, r) {
        if (!s)
          try {
            const o = JSON.parse(r.toString());
        ...
        s || n.toString().split(/\n\s*\n/).forEach((o) => {
          $e.getValue(o.split(`
`), "PNPClass", ":") === "Bluetooth" && e.push($0(o.split(`
`)));
        }), t && t(e), i(e);
      }), (k0 || W0 || F0 || R0) && i(null);
    });
...
to.bluetoothDevices = H0;
(function(t) {
  const i = _o.version, e = T, n = Pi, s = ei, r = Ct, o = ir, a = Qa, l = Gs, c = Bt, u = Vt, f = pn, p = fn, d = Ks, m = fr, h = Lt, x = Js, g = Qs, y = Zs, w = eo, S = to;
  let C = process.platform;
  const _ = C === "win32", O = C === "freebsd", U = C === "openbsd", b = C === "netbsd", W = C === "sunos";
    ...
    return new Promise((Z) => {
      process.nextTick(() => {
        ...
        H.version = V(), Promise.all([
          ...
          H.system = z[0], H.bios = z[1], H.baseboard = z[2], H.chassis = z[3], H.os = z[4], H.uuid = z[5], H.versions = z[6], H.cpu = z[7], H.cpu.flags = z[8], H.graphics = z[9], H.net = z[10], H.memLayout = z[11], H.diskLayout = z[12], $ && $(H), Z(H);
        });
      });
    });
  }
  function se($, Z, H) {
    return e.isFunction(Z) && (H = Z, Z = ""), e.isFunction($) && (H = $, $ = ""), new Promise((z) => {
      process.nextTick(() => {
        Z = Z || u.getDefaultNetworkInterface(), $ = $ || "";
        let B = function() {
          let G = 15;
          return _ && (G = 13), (O || U || b) && (G = 11), W && (G = 6), function() {
            --G === 0 && (H && H(K), z(K));
          };
        }(), K = {};
        K.time = s.time(), K.node = process.versions.node, K.v8 = process.versions.v8, r.cpuCurrentSpeed().then((G) => {
          ...
          K.temp = G, B();
        }), !U && !O && !b && !W && u.networkStats(Z).then((G) => {
          K.networkStats = G, B();
          ...
          K.fsSize = G, B();
        }), !_ && !U && !O && !b && !W && c.fsStats().then((G) => {
          K.fsStats = G, B();
        }), !_ && !U && !O && !b && !W && c.disksIO().then((G) => {
          K.disksIO = G, B();
        }), !U && !O && !b && !W && f.wifiNetworks().then((G) => {
          K.wifiNetworks = G, B();
  ...
  function le($, Z, H) {
    return new Promise((z) => {
      process.nextTick(() => {
        let B = {};
        Z && e.isFunction(Z) && !H && (H = Z, Z = ""), $ && e.isFunction($) && !Z && !H && (H = $, $ = "", Z = ""), ie().then((K) => {
          B = K, se($, Z).then((G) => {
            for (let Se in G)
  ...
  function X($, Z) {
    return new Promise((H) => {
      process.nextTick(() => {
        const z = Object.keys($).filter((B) => ({}).hasOwnProperty.call(t, B)).map((B) => {
          const K = $[B].substring($[B].lastIndexOf("(") + 1, $[B].lastIndexOf(")"));
          let G = B.indexOf(")") >= 0 ? B.split(")")[1].trim() : B;
          return G = B.indexOf("|") >= 0 ? B.split("|")[0].trim() : G, K ? t[G](K) : t[G]("");
        });
        Promise.all(z).then((B) => {
          const K = {};
          let G = 0;
          for (let Se in $)
            if ({}.hasOwnProperty.call($, Se) && {}.hasOwnProperty.call(t, Se) && B.length > G) {
              if ($[Se] === "*" || $[Se] === "all")
                K[Se] = B[G];
              else {
                let xe = $[Se], gn = "", kt = [];
                if (xe.indexOf(")") >= 0 && (xe = xe.split(")")[1].trim()), xe.indexOf("|") >= 0 && (gn = xe.split("|")[1].trim(), kt = gn.split(":"), xe = xe.split("|")[0].trim()), xe = xe.replace(/,/g, " ").replace(/ +/g, " ").split(" "), B[G])
                  if (Array.isArray(B[G])) {
                    const bt = [];
                    B[G].forEach((_t) => {
                      let Ot = {};
                      if (xe.length === 1 && (xe[0] === "*" || xe[0] === "all") ? Ot = _t : xe.forEach((Xe) => {
                        ({}).hasOwnProperty.call(_t, Xe) && (Ot[Xe] = _t[Xe]);
                      }), gn && kt.length === 2) {
                        if ({}.hasOwnProperty.call(Ot, kt[0].trim())) {
                          const Xe = Ot[kt[0].trim()];
                          typeof Xe == "number" ? Xe === parseFloat(kt[1].trim()) && bt.push(Ot) : typeof Xe == "string" && Xe.toLowerCase() === kt[1].trim().toLowerCase() && bt.push(Ot);
                        }
                      } else
                        bt.push(Ot);
                    }), K[Se] = bt;
                    ...
                    const bt = {};
                    xe.forEach((_t) => {
                      ({}).hasOwnProperty.call(B[G], _t) && (bt[_t] = B[G][_t]);
                    }), K[Se] = bt;
                  }
                else
                  K[Se] = {};
...
var Q0 = J0(oe), Qr = te, Z0 = (
  ...
  function() {
    ...
    return t.detectPlatform = function() {
      ...
      return Qr.execSync("chcp").toString().split(":")[1].trim();
    }, t.execute = function(i) {
      return Qr.execSync(i, { windowsHide: !0, encoding: "buffer" });
    }, t;
  }()
);
Nt.Utils = Z0;
...
dn.Darwin = ru;
var mn = {}, su = Te && Te.__importDefault || function(t) {
  return t && t.__esModule ? t : { default: t };
};
Object.defineProperty(mn, "__esModule", { value: !0 });
mn.Linux = void 0;
var ou = si, au = su(Ei), cu = Nt, lu = (
  /** @class */
  function() {
    function t() {
    }
...
mn.Linux = lu;
var hn = {}, io = { exports: {} }, zi = Lo, Xt = zi.Buffer, Ae = {}, Ee;
for (Ee in zi)
  zi.hasOwnProperty(Ee) && (Ee === "SlowBuffer" || Ee === "Buffer" || (Ae[Ee] = zi[Ee]));
var Kt = Ae.Buffer = {};
for (Ee in Xt)
  Xt.hasOwnProperty(Ee) && (Ee === "allocUnsafe" || Ee === "allocUnsafeSlow" || (Kt[Ee] = Xt[Ee]));
Ae.Buffer.prototype = Xt.prototype;
(!Kt.from || Kt.from === Uint8Array.from) && (Kt.from = function(t, i, e) {
  if (typeof t == "number")
...
Kt.alloc || (Kt.alloc = function(t, i, e) {
  if (typeof t != "number")
...
yr.prototype.write = function(t) {
  return this.addBOM && (t = no + t, this.addBOM = !1), this.encoder.write(t);
};
...
Sr.prototype.write = function(t) {
  var i = this.decoder.write(t);
  return this.pass || !i || (i[0] === no && (i = i.slice(1), typeof this.options.stripBOM == "function" && this.options.stripBOM()), this.pass = !0), i;
};
...
function pu() {
  ...
  e.prototype.write = function(c) {
    for (var u = t.from(c, "ucs2"), f = t.alloc(u.length * 2), p = this.isLE ? f.writeUInt32LE : f.writeUInt32BE, d = 0, m = 0; m < u.length; m += 2) {
      var h = u.readUInt16LE(m), x = 55296 <= h && h < 56320, g = 56320 <= h && h < 57344;
      if (this.highSurrogate)
        if (x || !g)
          p.call(f, this.highSurrogate, d), d += 4;
        else {
          var y = (this.highSurrogate - 55296 << 10 | h - 56320) + 65536;
          p.call(f, y, d), d += 4, this.highSurrogate = 0;
          continue;
        }
      x ? this.highSurrogate = h : (p.call(f, h, d), d += 4, this.highSurrogate = 0);
    }
  ...
  n.prototype.write = function(c) {
    if (c.length === 0)
      return "";
    var u = 0, f = 0, p = t.alloc(c.length + 4), d = 0, m = this.isLE, h = this.overflow, x = this.badChar;
    if (h.length > 0) {
      for (; u < c.length && h.length < 4; u++)
        h.push(c[u]);
      h.length === 4 && (m ? f = h[u] | h[u + 1] << 8 | h[u + 2] << 16 | h[u + 3] << 24 : f = h[u + 3] | h[u + 2] << 8 | h[u + 1] << 16 | h[u] << 24, h.length = 0, d = s(p, d, f, x));
    }
    for (; u < c.length - 3; u += 4)
      m ? f = c[u] | c[u + 1] << 8 | c[u + 2] << 16 | c[u + 3] << 24 : f = c[u + 3] | c[u + 2] << 8 | c[u + 1] << 16 | c[u] << 24, d = s(p, d, f, x);
    for (; u < c.length; u++)
      h.push(c[u]);
  ...
  function l(c, u) {
    var f = [], p = 0, d = 0, m = 0, h = 0, x = 0;
    e:
      for (var g = 0; g < c.length; g++)
        for (var y = c[g], w = 0; w < y.length; w++)
          if (f.push(y[w]), f.length === 4) {
            ...
            if ((f[0] !== 0 || f[1] > 16) && m++, (f[3] !== 0 || f[2] > 16) && d++, f[0] === 0 && f[1] === 0 && (f[2] !== 0 || f[3] !== 0) && x++, (f[0] !== 0 || f[1] !== 0) && f[2] === 0 && f[3] === 0 && h++, f.length = 0, p++, p >= 100)
              break e;
          }
    return x - m > h - d ? "utf-32be" : x - m < h - d ? "utf-32le" : u || "utf-32le";
  }
  return Ue;
}
var Wi = {}, ts;
  ...
  function a(l, c) {
    ...
    e:
      for (var m = 0; m < l.length; m++)
        for (var h = l[m], x = 0; x < h.length; x++)
          if (u.push(h[x]), u.length === 2) {
            if (f === 0) {
...
function du() {
  ...
  n.prototype.write = function(h) {
    return t.from(h.replace(e, (function(x) {
  ...
  s.prototype.write = function(h) {
    for (var x = "", g = 0, y = this.inBase64, w = this.base64Accum, S = 0; S < h.length; S++)
      if (!y)
        h[S] == l && (x += this.iconv.decode(h.slice(g, S), "ascii"), g = S + 1, y = !0);
      else if (!o[h[S]]) {
        if (S == g && h[S] == c)
          x += "+";
        else {
    ...
    if (!y)
      x += this.iconv.decode(h.slice(g), "ascii");
    else {
      var C = w + this.iconv.decode(h.slice(g), "ascii"), _ = C.length - C.length % 8;
      w = C.slice(_), C = C.slice(0, _), x += this.iconv.decode(t.from(C, "base64"), "utf16-be");
    }
    return this.inBase64 = y, this.base64Accum = w, x;
  }, s.prototype.end = function() {
    var h = "";
  ...
  p.prototype.write = function(h) {
    for (var x = this.inBase64, g = this.base64Accum, y = this.base64AccumIdx, w = t.alloc(h.length * 5 + 10), S = 0, C = 0; C < h.length; C++) {
      var _ = h.charCodeAt(C);
      32 <= _ && _ <= 126 ? (x && (y > 0 && (S += w.write(g.slice(0, y).toString("base64").replace(/\//g, ",").replace(/=+$/, ""), S), y = 0), w[S++] = c, x = !1), x || (w[S++] = _, _ === u && (w[S++] = c))) : (x || (w[S++] = u, x = !0), x && (g[y++] = _ >> 8, g[y++] = _ & 255, y == g.length && (S += w.write(g.toString("base64").replace(/\//g, ","), S), y = 0)));
    }
    return this.inBase64 = x, this.base64AccumIdx = y, w.slice(0, S);
  }, p.prototype.end = function() {
    var h = t.alloc(10), x = 0;
    return this.inBase64 && (this.base64AccumIdx > 0 && (x += h.write(this.base64Accum.slice(0, this.base64AccumIdx).toString("base64").replace(/\//g, ",").replace(/=+$/, ""), x), this.base64AccumIdx = 0), h[x++] = c, this.inBase64 = !1), h.slice(0, x);
  };
  function d(h, x) {
    this.iconv = x.iconv, this.inBase64 = !1, this.base64Accum = "";
  }
  ...
  return m[44] = !0, d.prototype.write = function(h) {
    for (var x = "", g = 0, y = this.inBase64, w = this.base64Accum, S = 0; S < h.length; S++)
      if (!y)
        h[S] == u && (x += this.iconv.decode(h.slice(g, S), "ascii"), g = S + 1, y = !0);
      else if (!m[h[S]]) {
        if (S == g && h[S] == c)
          x += "&";
        else {
...
function xu() {
  ...
  function l(p, d) {
    ...
    if (typeof p.gb18030 == "function") {
      ...
      for (var y = this.decodeTables[0], h = 129; h <= 254; h++)
        for (var w = this.decodeTables[s - y[h]], S = 48; S <= 57; S++) {
          if (w[S] === i)
            w[S] = s - x;
          else if (w[S] > s)
    ...
    if (this._fillEncodeTable(0, 0, b), p.encodeAdd)
  ...
  l.prototype.encoder = c, l.prototype.decoder = u, l.prototype._getDecodeTrieNode = function(p) {
    ...
    var m = p[0], h = this._getEncodeBucket(m), x = m & 255, g;
    h[x] <= n ? g = this.encodeTableSeq[n - h[x]] : (g = {}, h[x] !== i && (g[o] = h[x]), h[x] = n - this.encodeTableSeq.length, this.encodeTableSeq.push(g));
    ...
    for (var h = this.decodeTables[p], x = !1, g = {}, y = 0; y < 256; y++) {
      var w = h[y], S = d + y;
      if (!m[S])
        if (w >= 0)
          this._setEncodeChar(w, S), x = !0;
        else if (w <= s) {
          var C = s - w;
          if (!g[C]) {
            var _ = S << 8 >>> 0;
            this._fillEncodeTable(C, _, m) ? x = !0 : g[C] = !0;
          }
        } else w <= n && (this._setEncodeSequence(this.decodeTableSeq[n - w], S), x = !0);
    }
    return x;
  };
  function c(p, d) {
  ...
  c.prototype.write = function(p) {
    for (var d = t.alloc(p.length * (this.gb18030 ? 4 : 3)), m = this.leadSurrogate, h = this.seqObj, x = -1, g = 0, y = 0; ; ) {
      if (x === -1) {
        if (g == p.length) break;
      ...
      if (h !== void 0 && w != i) {
        ...
        if (typeof C == "object") {
          h = C;
          continue;
        } else typeof C == "number" ? S = C : C == null && (C = h[o], C !== void 0 && (S = C, x = w));
        h = void 0;
      } else if (w >= 0) {
        ...
        if (S == i && this.gb18030) {
          ...
          if (O != -1) {
            ...
            d[y++] = 129 + Math.floor(S / 12600), S = S % 12600, d[y++] = 48 + Math.floor(S / 1260), S = S % 1260, d[y++] = 129 + Math.floor(S / 10), S = S % 10, d[y++] = 48 + S;
            continue;
          }
        }
      }
      S === i && (S = this.defaultCharSingleByte), S < 256 ? d[y++] = S : S < 65536 ? (d[y++] = S >> 8, d[y++] = S & 255) : S < 16777216 ? (d[y++] = S >> 16, d[y++] = S >> 8 & 255, d[y++] = S & 255) : (d[y++] = S >>> 24, d[y++] = S >>> 16 & 255, d[y++] = S >>> 8 & 255, d[y++] = S & 255);
    }
    return this.seqObj = h, this.leadSurrogate = m, d.slice(0, y);
  ...
  u.prototype.write = function(p) {
    for (var d = t.alloc(p.length * 2), m = this.nodeIdx, h = this.prevBytes, x = this.prevBytes.length, g = -this.prevBytes.length, y, w = 0, S = 0; w < p.length; w++) {
      var C = w >= 0 ? p[w] : h[w + x], y = this.decodeTables[m][C];
      if (!(y >= 0)) if (y === i)
        y = this.defaultCharUnicode.charCodeAt(0), w = g;
      else if (y === e) {
        if (w >= 3)
          var _ = (p[w - 3] - 129) * 12600 + (p[w - 2] - 48) * 1260 + (p[w - 1] - 129) * 10 + (C - 48);
        else
          var _ = (h[w - 3 + x] - 129) * 12600 + ((w - 2 >= 0 ? p[w - 2] : h[w - 2 + x]) - 48) * 1260 + ((w - 1 >= 0 ? p[w - 1] : h[w - 1 + x]) - 129) * 10 + (C - 48);
        var O = f(this.gb18030.gbChars, _);
        y = this.gb18030.uChars[O] + _ - this.gb18030.gbChars[O];
      } else if (y <= s) {
        m = s - y;
        continue;
      } else if (y <= n) {
        for (var U = this.decodeTableSeq[n - y], b = 0; b < U.length - 1; b++)
          y = U[b], d[S++] = y & 255, d[S++] = y >> 8;
        y = U[U.length - 1];
      } else
        throw new Error("iconv-lite internal error: invalid decoding table value " + y + " at " + m + "/" + C);
      if (y >= 65536) {
        y -= 65536;
        var W = 55296 | y >> 10;
        d[S++] = W & 255, d[S++] = W >> 8, y = 56320 | y & 1023;
      }
      d[S++] = y & 255, d[S++] = y >> 8, m = 0, g = w + 1;
    }
    return this.nodeIdx = m, this.prevBytes = g >= 0 ? Array.prototype.slice.call(p, g) : h.slice(g + x).concat(Array.prototype.slice.call(p)), d.slice(0, S).toString("ucs2");
...
(function(t) {
  var i = It.Buffer, e = xr, n = t.exports;
...
var Pu = io.exports, ro = Te && Te.__importDefault || function(t) {
  return t && t.__esModule ? t : { default: t };
};
Object.defineProperty(hn, "__esModule", { value: !0 });
hn.Windows = void 0;
var Au = si, Eu = ro(Ei), fs = Nt, ds = ro(Pu), Mu = (
  /** @class */
  function() {
    function t() {
    }
    return t.run = function() {
      var i = [], e = fs.Utils.execute(Au.Constants.WINDOWS_COMMAND), n = fs.Utils.chcp(), s = "";
      switch (n) {
        case "65000":
          s = "UTF-7";
          break;
      ...
      e = ds.default.encode(ds.default.decode(e, s), "UTF-8");
...
`), o = !1, a = "", l = "", c = 0, u = 0;
      return r.forEach(function(f) {
        if (f !== "") {
          ...
          var h = u - c, x = "0%";
          u > 0 && (x = Math.round(h / u * 100) + "%");
          var g = new Eu.default(l, u, h, c, x, a);
...
Zt.getDiskInfoSync = Zt.getDiskInfo = void 0;
var Yt = dn, so = mn, oo = hn, ao = Nt;
function Tu() {
  return new Promise(function(t, i) {
    try {
      var e = ao.Utils.detectPlatform(), n = void 0;
      switch (e) {
        case "aix":
...
Zt.getDiskInfo = Tu;
function Du() {
  var t = ao.Utils.detectPlatform(), i;
  switch (t) {
    case "aix":
    ...
    case "android":
      throw new Error("Platform not supported: " + t);
    case "darwin":
      return i = Yt.Darwin.run(), i;
    case "freebsd":
      return i = Yt.Darwin.run(), i;
    case "linux":
      return i = so.Linux.run(), i;
    case "openbsd":
      return i = Yt.Darwin.run(), i;
    case "sunos":
      throw new Error("Platform not supported: " + t);
    case "win32":
      return i = oo.Windows.run(), i;
    default:
      throw new Error("Platform not recognized: " + t);
  }
}
Zt.getDiskInfoSync = Du;
class Bu {
  /**
   ...
   */
  async getCpuInfo() {
    const i = oe.cpus()[0], e = oe.cpus().length, n = oe.cpus().length;
    return {
      model: i.model,
   ...
   */
  getMemoryInfo() {
    const i = oe.totalmem(), e = oe.freemem();
    return {
  ...
  async getDiskInfo() {
    ...
    try {
      const n = Zt.getDiskInfoSync(), s = n.find((r) => r.mounted === "C:") || n[0];
  ...
  getOsInfo() {
    return {
      os: `${oe.type()} ${oe.arch()}`,
      version: oe.release(),
      networkName: oe.networkInterfaces().Ethernet ? "Ethernet" : "Wi-Fi"
    };
  }
  /**
  ...
  getScreenResolution() {
    let i = "Bilinmiyor";
    try {
      const e = mo.getPrimaryDisplay(), { width: n, height: s } = e.size;
      i = `${n}x${s}`;
    } catch (e) {
   ...
   */
  getUptime() {
    const i = oe.uptime(), e = Math.floor(i / 3600), n = Math.floor(i % 3600 / 60);
  ...
  async getAllSystemInfo() {
    const i = await this.getCpuInfo(), e = this.getMemoryInfo(), n = await this.getDiskInfo(), s = this.getOsInfo(), r = this.getScreenResolution(), o = await this.getBatteryLevel(), a = this.getUptime();
    return {
      cpu: i.model,
      cpuSpeed: i.speed,
      cpuCores: i.cores,
      ...
      for (let n = 1; n <= 3; n++)
        try {
          return await new Promise((r, o) => {
            ...
            const a = wr("winget", [i, ...e], {
              shell: !0
            });
            let l = "", c = "";
            a.stdout.on("data", (f) => {
              const p = f.toString();
              l += p, console.log(`[winget stdout]: ${p}`);
            }), a.stderr.on("data", (f) => {
              const p = f.toString();
              c += p, console.error(`[winget stderr]: ${p}`);
            }), a.on("close", (f) => {
              if (console.log(`Winget command exited with code ${f}`), f !== 0) {
                const p = c || `Winget komutu ${f} kodu ile başarısız oldu`, d = {
                  command: `winget ${i} ${e.join(" ")}`,
                  exitCode: f,
                  stdout: l,
                  stderr: c
                };
                console.error("Winget command failed:", d), c.includes("0x80070002") || c.includes("not found") ? o(new Error("Winget bulunamadı. Lütfen winget'in yüklü olduğundan emin olun")) : c.includes("0x8007139f") ? o(new Error("Bu işlem için yönetici izinleri gerekli olabilir")) : c.includes("NetworkError") || c.includes("network") ? o(new Error("Ağ hatası: Lütfen internet bağlantınızı kontrol edin")) : c.includes("Failed to update package") ? o(new Error("Paket güncellenirken hata oluştu: Güncelleme engellenmiş veya paket kilitli olabilir")) : o(new Error(p));
              } else
                l.trim() ? ((l.includes("failed") || l.includes("error")) && console.warn("Winget command output contains error indicators:", l), r(l)) : (console.warn("Winget command returned empty output"), r("Komut başarıyla çalıştı fakat çıktı üretmedi"));
            }), a.on("error", (f) => {
              console.error("Winget process error:", f), o(new Error(`Winget komutu çalıştırılamadı: ${f.message}`));
            });
            const u = setTimeout(() => {
              a.kill(), console.error("Winget command timed out after 30 seconds"), o(new Error("Winget komutu zaman aşımına uğradı. İşlem iptal edildi."));
            }, 3e4);
            a.on("close", () => clearTimeout(u));
          });
        } catch (s) {
          if (n === 3)
            throw console.error(`Winget command failed after 3 attempts: ${s.message}`), s;
          const r = n * 2e3;
          console.log(`Winget komutu başarısız oldu, ${n}/3 yeniden deneniyor (${r}ms sonra)...`), await new Promise((o) => setTimeout(o, r));
        }
...
class Nu {
  ...
  registerSystemInfoHandlers() {
    oi.handle("getSystemInfo", async () => await this.systemInfoService.getAllSystemInfo());
  }
  /**
   * Winget ile ilgili IPC işleyicilerini kaydeder
   */
  registerWingetHandlers() {
    oi.handle("get-installed-apps", async () => {
      try {
    ...
    }), oi.handle("search-apps", async (i, e) => {
      try {
        return await this.wingetService.searchApps(e);
      } catch (n) {
        return console.error("IPC handler search-apps error:", n), { error: n.message || "Uygulama araması yapılırken bir hata oluştu" };
      }
    }), oi.handle("update-app", async (i, e) => {
      try {
        return { success: !0, message: await this.wingetService.updateApp(e) };
      } catch (n) {
    ...
    }), oi.handle("install-app", async (i, e) => {
      try {
        return { success: !0, message: await this.wingetService.installApp(e) };
      } catch (n) {
...
ho(import.meta.url);
const co = tt.dirname(go(import.meta.url));
process.env.APP_ROOT = tt.join(co, "..");
const jn = process.env.VITE_DEV_SERVER_URL, Zu = tt.join(process.env.APP_ROOT, "dist-electron"), lo = tt.join(process.env.APP_ROOT, "dist");
process.env.VITE_PUBLIC = jn ? tt.join(process.env.APP_ROOT, "public") : lo;
let ze;
function uo() {
  ze = new ms({
    width: 1200,
    height: 830,
    resizable: !0,
    icon: tt.join(process.env.VITE_PUBLIC, "icon.svg"),
    webPreferences: {
      preload: tt.join(co, "preload.mjs")
    }
  }), ze.setMenu(null), new Nu().registerHandlers(), ze.webContents.on("did-finish-load", () => {
    console.log("Page Loaded"), ze == null || ze.webContents.send("main-process-message", (/* @__PURE__ */ new Date()).toLocaleString());
  }), jn ? ze.loadURL(jn) : ze.loadFile(tt.join(lo, "index.html"));
}
wi.on("window-all-closed", () => {
  process.platform !== "darwin" && (wi.quit(), ze = null);
});
wi.on("activate", () => {
  ms.getAllWindows().length === 0 && uo();
});
wi.whenReady().then(uo);
export {
  Zu as MAIN_DIST,
  lo as RENDERER_DIST,
  jn as VITE_DEV_SERVER_URL
};
